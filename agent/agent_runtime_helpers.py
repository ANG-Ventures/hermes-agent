...
AGENT_RUNTIME_POST_HOOK_TOOL_NAMES = frozenset(
    # Parity 2026-08-29: ``message_agent`` (Bot Mode teammate DM) is dispatched
    # inline in execute_tool_calls_sequential exactly like its siblings — same
    # _run_agent_tool_execution_middleware wrapper — so it emits its own
    # post_tool_call hook and must be declared here. Upstream added the inline
    # branch without updating this set; the fork-only drift guard
    # (tests/run_agent/test_run_agent_tool_exec.py::
    # TestAgentRuntimePostHookOwnershipSync) is what caught it.
    {"todo", "session_search", "memory", "clarify", "read_terminal", "desktop_preview", "drive_preview", "annotate_preview", "read_window_below", "setup_mcp", "tour", "delegate_task", "message_agent"}
)
...
def repair_message_sequence(agent, messages: List[Dict]) -> int:
    ...
    Repairs applied:
      ...
      1.5. ``tool_calls`` on an assistant message that no immediately
         following ``tool`` result answers (an UNANSWERED tool_use — a
         guaranteed provider 400) are pruned COUNT-matched per alias group;
         the turn is dropped entirely if that leaves it payload-empty (an
         empty non-final assistant message is itself a 400 on most
         providers). This catches a duplicate/orphan buried MID-history
         (e.g. an FTS write-corruption / restart-storm double-write, a
         host-fed broken history, or a result displaced by compression)
         that the dangling-TAIL stripper and Pass 0 both miss.
    ...
    # tool call, AND drop SURPLUS duplicate results beyond the number of
    # calls issued (fork #58327/#196 count semantics). A Responses call can
    # have several equivalent spellings (call_id, id, response_item_id, or a
    # composite ``call|item`` id), so consume the whole alias GROUP when one
    # spelling is matched — otherwise a duplicate result keyed on the sibling
    # alias would survive and be replayed to strict providers (#66974).
    # Alias expansion lives in ``agent.message_sanitization
    # .tool_call_id_variants`` / ``tool_result_id_variants`` (single policy
    # owner) — which also handles SDK tool_call objects, preserving the
    # #91768 dict-or-object tolerance.
    #
    # COUNT semantics (fork parity, Greptile P1 on #196): each tool_call is
    # its own group even when its id duplicates a sibling's, so a turn with
    # calls ``[X, X]`` has TWO answer slots for X — two results keep both,
    # a THIRD is surplus. A variant therefore maps to a LIST of group ids.
    known_tool_ids: Dict[str, List[int]] = {}
        ...
        if role == "assistant":
            ...
            for tc in (msg.get("tool_calls") or []):
                ...
                for tc_id in variants:
                    known_tool_ids.setdefault(tc_id, []).append(group_id)
            filtered.append(msg)
        elif role == "tool":
            ...
            candidate_groups = {
                ...
                for group_id in known_tool_ids.get(tc_id, [])
                if group_id not in matched_tool_groups
            }
            if not result_variants:
                filtered.append(msg)
            elif candidate_groups:
                # Consume ONE alias group (one answer slot) so a SECOND result
                # replaying any sibling spelling — or a SURPLUS duplicate for
                # the same id beyond the number of calls issued — falls into
                # the drop branch below. Strict providers reject duplicate
                # tool_call_ids with HTTP 400 (#58327, #66974); Anthropic 400s
                # multiple tool_result blocks per tool_use (#196). Credit: #55436.
    ...
    # Pass 1.5 / Pass 2 (fork + upstream converged): prune tool_calls that
    # were never answered positionally.
    #
    # Pass 1 dropped the stray/displaced tool RESULT — but a tool_call whose
    # result was displaced far beyond the following turn (context compression
    # can move it past a user turn; an FTS write-corruption / restart-storm
    # double-write can orphan a twin MID-history — the 2026-07-04 incident)
    # leaves its declaring assistant message carrying an UNANSWERED
    # tool_call, and strict providers reject the payload with HTTP 400
    # ("assistant message with 'tool_calls' must be followed by tool
    # messages..." on OpenAI/DeepSeek/Kimi; "`tool_use` ids were found
    # without `tool_result` blocks immediately after" on Anthropic).
    # Enforce the positional invariant: a tool_call is only legitimate when
    # a result for ANY of its id variants (same unified alias policy as
    # Pass 1) appears in the run of tool messages IMMEDIATELY following the
    # declaring assistant message. COUNT-matched, not set-matched (fork
    # Greptile P1 on #196): each result is ONE answer slot, so a turn with
    # duplicate ids ``[X, X]`` answered by a single result for X keeps ONE
    # call, not both. Unanswered calls are pruned; if the message then
    # carries no other payload the whole turn is dropped — an empty
    # non-final assistant message is itself rejected by providers. Codex
    # interim turns are exempt, as in Pass 0: their calls are replayed
    # through the Responses-items chain, not the tool-result run.
    pruned: List[Dict] = []
    i = 0
    n = len(filtered)
    while i < n:
        ...
        if not (
            ...
            i += 1
            continue
        # Collect the contiguous tool-result run as ANSWER SLOTS: each result
        # is one slot carrying its variant set. A call consumes the first
        # unconsumed slot whose variants intersect its own — count-correct
        # for duplicate ids AND alias-correct for Responses spellings.
        slots: List[set] = []
        j = i + 1
        while (
            j < n
            ...
            if tid:
                slots.append(set(tool_result_id_variants(tid)))
            j += 1
        ...
        for tc in msg.get("tool_calls") or []:
            ...
            if variants:
                for k, slot in enumerate(slots):
                    if not consumed[k] and (variants & slot):
                        consumed[k] = True
                        matched = True
                        break
            if matched:
        ...
        if dropped_calls:
            ...
            if not kept_calls and not _msg_has_payload(
                ...
                logger.debug(
                    ...
                    "turn with %d unanswered call(s) and no other payload",
                    dropped_calls,
                )
                i += 1
                continue
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if function_name == "todo":
        ...
        # registered: only a canonical Bot Chat session carries the schema,
        # and the tool re-gates on the session title itself.
        #
        # Parity 2026-08-29: upstream added this branch to the SEQUENTIAL
        # dispatcher (tool_executor.execute_tool_calls_sequential) only. The
        # concurrent path needs the same branch or a message_agent call issued
        # in a parallel tool batch falls through to the registry, which has no
        # such tool. The fork-only drift guard
        # (TestAgentRuntimePostHookOwnershipSync) is what caught the asymmetry.
        def _execute(next_args: dict) -> Any:
