...
def repair_message_sequence(agent, messages: List[Dict]) -> int:
    ...
    # tool_call_id, AND drop SURPLUS duplicate results beyond the number of
    # calls issued for an id. Uses a rolling per-id answer BUDGET (count)
    # refreshed on each assistant message.
    #
    ...
    for msg in collapsed:
        ...
        if role == "assistant":
            ...
            if tc_id and known_tool_budget.get(tc_id, 0) > 0:
                ...
                filtered.append(msg)
                # A SECOND tool result carrying the same tool_call_id
                # (duplicate from a retry/crash/session-resume glitch) now
                # falls into the drop branch below because the budget for
                # this id is exhausted — strict providers (DeepSeek) reject a
                # duplicate tool_call_id with HTTP 400 (#58327). Credit: #55436.
            else:
