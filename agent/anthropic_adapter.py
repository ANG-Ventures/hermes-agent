...
def _convert_content_to_anthropic(
    content: Any, placeholder_text: Optional[str] = None
) -> Any:
    """Convert OpenAI-style multimodal content arrays to Anthropic blocks.

    ``placeholder_text`` is the all-blank fallback. It is role-dependent
    upstream ("(empty)" for assistant, "(empty message)" for user), and this
    filter runs BEFORE upstream's ``_fix_blank_text_blocks_in_list`` sweep,
    so the caller must pass the role-correct value or the sweep can no longer
    correct it (the block is non-blank by then).
    """
    if not isinstance(content, list):
        return content

    if placeholder_text is None:
        placeholder_text = _EMPTY_TEXT_PLACEHOLDER
    converted = []
    ...
    for part in content:
        ...
        if (
            ...
            # parity 2026-08-07: the fork's blank-block filter and upstream's
            # relocation logic in _convert_assistant_message are two independent
            # guards for the same incident class. This filter runs FIRST, so a
            # cache_control marker riding the dropped block would never reach
            # upstream's relocator and would be silently lost — costing a
            # prompt-caching breakpoint on every affected turn. Carry it here.
            if isinstance(block.get("cache_control"), dict):
                relocated_cache_control = block["cache_control"]
            continue
    ...
    if not converted and content:
        ...
        converted = [{"type": "text", "text": placeholder_text}]
    if relocated_cache_control is not None and converted:
        _apply_assistant_cache_control_to_last_cacheable_block(
            converted, relocated_cache_control
        )
    return converted
...
def _convert_user_message(content: Any) -> Dict[str, Any]:
    ...
    if isinstance(content, list):
        converted_blocks = _convert_content_to_anthropic(
            content, placeholder_text="(empty message)"
        )
