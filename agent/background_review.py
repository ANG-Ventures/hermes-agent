...
# Iteration budget for the review fork: see _REVIEW_MAX_ITERATIONS at the top
...
def _run_review_in_thread(
    ...
    try:
        ...
        with thread_scoped_silence():
            ...
            # Fork parity: chat identity for blackbox/session attribution (the
            # helper doesn't thread constructor chat kwargs).
            review_agent._chat_id = getattr(agent, "_chat_id", None) or ""
            review_agent._chat_name = getattr(agent, "_chat_name", None) or ""
            review_agent._chat_type = getattr(agent, "_chat_type", None) or ""
            ...
            # this, the fork shares the parent's session_id (pinned by the
            # cache-parity helper) → identical bridge routing key → the fork's
            ...
            # mem0-in-background-review (default-OFF, fork): when the flag is on,
            # allow the manager-free mem0 write helper (registry tool in the
            # 'memory_write' toolset, so it's parent-resident/cache-stable but
            # denied here unless explicitly whitelisted — denied-not-absent). See
            ...
            try:
                ...
                if request_admitted:
                    ...
                    # whitelist: when the mem0 flag is on, mem0_remember IS allowed,
                    # so a blanket "only memory and skill tools" reminder would
                    # contradict the mem0 clause in `prompt` and suppress the very
                    # call we want (the model obeys the last, most-emphatic
                    # instruction). Keep them consistent. Configured extra tools
                    # are named too (upstream #44672).
                    if _bgr_mem0_on:
                        _tool_reminder = (
                ...
                # LOUD on budget exhaustion (fork). The fork runs with
