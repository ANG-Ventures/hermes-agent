    ...
    *,
    ...
    if reason in {FailoverReason.rate_limit, FailoverReason.billing,
        ...
        if (not fallback_already_active) or (primary_provider and current_provider == primary_provider):
            # Provider-supplied window (reset_at) is the floor, via the shared
            # _primary_cooldown_seconds; on top of it, escalate on CONSECUTIVE
            # rate-limits: 1x → 2x → 4x → 8x → ... → 4h cap. The first 429 must
            # NOT bench the primary for half an hour — fast primary restore is
            # the common case; escalation only punishes providers that keep
            # 429ing. Counter is reset by restore_primary_runtime on a
            # successful restore.
    ...
    try:
        ...
        # (Upstream's list-based ``_pending_fallback_notice`` re-creates the
        # same duplicate-line class beside the announce, so it is not adopted.)
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        for chunk in _iter_provider_stream_chunks(
            ...
            # completion message always receive plain text.  The flattened text
            # then runs through the surrogate splicer so UTF-16 pairs split
            # across deltas recombine before display/accumulation.
            delta_content = flatten_message_text(getattr(delta, "content", None), sep="")
            if delta_content:
                content_delta = content_splicer.feed(delta_content)
                if content_delta:
                    content_parts.append(content_delta)
                    if not tool_calls_acc:
                        if pending_text_parts or _provider_stream_text_may_be_sse(content_delta):
                            pending_text_parts.append(content_delta)
    ...
    def _call_anthropic(request_client):
        ...
        if agent._interrupt_requested:
            return None
        # Shim variants of the zero-event failure: an OpenAI-compat adapter may
        # fabricate a contentless Message with no stop_reason, or return None
        # where the SDK assert would have fired (e.g. ``python -O``). A real
        # completed response always carries a stop_reason. A None base falls
        # through to the accumulator below and is caught by the same guard.
