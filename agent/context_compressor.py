...
class ContextCompressor(ContextEngine):
    ...
    def _record_ineffective_compression_verdict(self, count: int) -> None:
        ...
        """
        if count == getattr(self, "_ineffective_compression_count", None):
            return
    ...
    def update_model(
        ...
        # Re-resolve the threshold for the DESTINATION model on a mid-session
        # switch/fallback. Chain BOTH resolvers to preserve both features:
        # fork's resolve_compression_threshold (config per_model_threshold +
        # built-in/Codex gpt-5.5 autoraise + global) provides the base, then
        # upstream's resolve_model_threshold (config compression.model_thresholds
        # substring map, longest match wins) overrides on top — matching the
        # init precedence in __init__ (2026-06-19 compaction-thrash fix +
        # Codex autoraise must survive opus 1M -> gpt-5.5 272K fallback).
        if self._per_model_threshold_cfg is not None or self._global_threshold_percent is not None:
            ...
            _config_pct = resolve_compression_threshold(
                ...
                global_threshold=(
                    ...
                    else getattr(self, "_config_threshold_percent", self.threshold_percent)
                ),
        ...
        # Upstream's substring model_thresholds map overrides the config/autoraise
        # base, then the small-context floor is applied for the NEW window.
        _new_base = resolve_model_threshold(
            model, getattr(self, "model_thresholds", None), _config_pct,
        )
    ...
    def _apply_threshold_tokens_cap(self) -> None:
        ...
        """
        _cap = getattr(self, "threshold_tokens_cap", None)
    ...
    def __init__(
        ...
        # Upstream d153918f1: session binding for compression-failure cooldown.
