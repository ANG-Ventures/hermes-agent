...
def _adopt_live_compression_child(
    ...
    recovered = loader(
        session_db,
        child_session_id,
        # parity 2026-08-30: the adoption/twin-scan path needs the durable
        # per-row timestamps (upstream carries them unconditionally; the
        # fork gates them for display/resume). Transport strips the key
        # before any model payload (#47868, chat_completions.py).
        ...
        if not in_place and _lock_db is not None and _lock_sid:
            ...
            if callable(durable_loader):
                durable_parent = durable_loader(
                    _lock_db, _lock_sid, include_timestamp=True  # parity 2026-08-30 (see loader note)
                )
            ...
            if commit_fence is not None and commit_fence.is_cancelled:
                ...
                with aux_progress_hook(_progress_hook), aux_interrupt_protection(
                    ...
                    try:
                        ...
                        # does NOT accept focus_topic / force / memory_context — fall
                        # back to the oldest documented call shape (current_tokens
                        # only). Fork behavior preserved on top of upstream's
                        # signature-filtering: the filter handles inspectable plugins,
                        # this catches the rest (#69870 lock refresher must still stop
                        # on the fallback path).
                        #
                        # BUT: when the signature was fully inspectable AND declared no
                        # **kwargs catch-all, the filter PROVED every kwarg we passed
                        # binds — so this TypeError came from INSIDE the engine, not
                        # from argument binding. Retrying would execute a stateful
                        # compressor a second time (the hazard
                        # _supported_compression_kwargs exists to prevent). Propagate
                        # it untouched; the outer BaseException handler still releases
                        # the lock and stops the refresher.
            ...
            try:
                ...
                if in_place:
                    ...
                    try:
                        ...
                        # The child could not be published (e.g. lost compression
                        # lease, FK constraint, contended write). Previously the
                        # outer handler simply warned and let the agent continue on
                        # the NEW id — which has no row in state.db, producing an
                        # orphan: the parent is ended, the child is never indexed,
                        # and every subsequent message is attributed to a session
                        # that does not exist (#33906/#33907). Roll the live id back
                        # to the parent so the conversation stays attached to a real,
                        # indexed session instead of a phantom.
                        #
                        # Parity note (2026-08-07): upstream replaced the fork's
                        # separate create_session with this ATOMIC publish, so the
                        # rollback is re-homed onto it. The redundant follow-up
                        # create_session was removed (publish already inserts the
                        # child row in the same transaction).
                        logger.warning(
                            "Compression child session publish failed (%s) — "
                            "rolling back to parent session %s to avoid an orphan.",
                            _cs_err, old_session_id,
                        )
                        agent.session_id = old_session_id
                        # The compacted list exists in memory but was NOT persisted
                        # (we rolled back to the parent). Mark it so the gateway
                        # reports a retryable save failure instead of a benign
                        # "No changes" no-op (#44794).
                        persist_failed = True
                        try:
                            from gateway.session_context import set_current_session_id
                        ...
                        try:
                            ...
                            pass
                        # Re-open the parent: it may have been ended by the
                        # transaction, but we are continuing on it.
                        try:
                            agent._session_db.reopen_session(old_session_id)
                        except Exception:
                            pass
                        # Parity note (2026-08-07): upstream performs this
                        # transcript rollback in the OUTER handler, guarded on
                        # ``old_session_id`` still being set. This fork-only inner
                        # handler clears that variable before re-raising, which
                        # starves the outer restoration. Re-graft it here, where
                        # the rollback actually happens, so both contracts hold:
                        # the parent stays live (fork #33906/#33907) AND the
                        # in-memory transcript + prune runway roll back with it
                        # (upstream). Without this the caller receives the stale
                        # compacted list for a rotation that never landed.
                        ...
                        # for pre-commit cancels, while this branch is
                        # post-attempt -- the other snapshot fields must keep the
                        # failed attempt's values. compress() zeroed the runway
                        # in-memory on summary success; the durable clear only
                        # rides the publication that just failed.
                        if "_proactive_prune_rearm_tokens" in _compressor_attempt_snapshot:
                            agent.context_compressor._proactive_prune_rearm_tokens = (
                                _compressor_attempt_snapshot[
                                    "_proactive_prune_rearm_tokens"
