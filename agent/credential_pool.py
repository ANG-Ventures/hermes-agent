...
class CredentialPool(CredentialPoolAdminMixin, CredentialPoolModelCooldownMixin):
    ...
    def _adopt_rotated_entry_unlocked(
        ...
        process consumed the single-use refresh token and wrote T2 to
        ...
        other writers hold, and the entry is not the one that failed.
        ...
        if self._entry_needs_refresh(candidate):
            # The entry's token is itself expiring; handing it back would just
            # trade one dead key for another. Let the refresh/fallback path own it.
            return None
        logger.info(
            "credential pool: failed %s key matches no entry but entry %s "
            "holds a different, unexpired token (rotated by another process) "
            "— adopting it for one retry",
            self.provider,
    ...
    def _rotate_unmatched(self, api_key_hint: Optional[str] = None) -> Optional[PooledCredential]:
        ...
        if next_entry is not None and len(self._available_entries()[0]) == 1:
            ...
            # underneath this agent: with single-use OAuth refresh another
            # process re-mints the pair, the pool picks up the new token via
            # its auth.json sync, and the agent still dispatches with the key
            # it was built with (the one that just failed and matches
            # nothing). Handing back the lone entry then DOES change the
            # credential, so try it once; the streak cap above bounds it.
            ...
            # recovery without changing the credential, and the caller
            # retries the same 401 indefinitely.
            self._unmatched_rotation_streak = 0
            self._current_id = None
            return None
        return next_entry
    ...
    def try_refresh_matching(
        ...
        with self._lock:
            ...
            if (
                ...
                # that failed: another process rotated the single-use pair
                # underneath this agent. Force-refreshing would burn the fresh
                # refresh token every other holder relies on, for a failure
                # this token never had. Adopt the entry's current token.
