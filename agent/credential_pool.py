            ...
            if self.provider == "anthropic":
                ...
                # a second authority for the same refresh-token family.
                    ...
                    try:
                        ...
                        committed_at = _write_hermes_oauth_credentials(
                            refreshed["access_token"],
                            refreshed["refresh_token"],
                            refreshed["expires_at_ms"],
                        )
                    ...
                    # Carry the instant the commit stamped onto the entry: the
                    # seeding gate compares the two, and an unstamped entry
                    # makes every comparison None (adopt-always).
            ...
            if creds and creds.get("accessToken"):
                ...
                changed |= _upsert_entry(
                    ...
                    source_name,
                    {
                        ...
                        # Carry the singleton's refresh instant into the entry
                        # under the pool's snake_case name, so the freshness
                        # gate has the same clock on both sides.  Absent on
                        # singletons written before the producer started
                        # stamping (and always absent for claude_code, whose
                        # writer is the Claude Code CLI); the gate stays
                        # adopt-by-default for those.
