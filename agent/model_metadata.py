        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if resp.status_code == 200:
                ...
                if matched is not None:
                    ...
                    #
                    # Parity note (2026-08-07): ``max_tokens`` is deliberately
                    # ABSENT from this list (upstream included it). On an
                    # Anthropic-style /v1/models passthrough ``max_tokens`` is the
                    # max *output* tokens (e.g. 128000 for a 1M-context model),
                    # NOT the context window — reading it collapses the window and
                    # triggers premature auto-compaction. See the same exclusion
                    # documented on the /v1/models/{model} branch above.
                    # ``max_input_tokens`` / ``max_position_embeddings`` (fork) ARE
                    # genuine input ceilings, so they stay.
