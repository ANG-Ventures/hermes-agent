...
from typing import AbstractSet, Any, Dict, List, Optional, Tuple, TYPE_CHECKING
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, dict):
                    ...
                    #
                    # Parity note (2026-08-29): this REPLACES the fork's
                    # hand-rolled key tuple. Upstream generalized the fork's own
                    # max_tokens exclusion into the shared helper, and
                    # _CONTEXT_LENGTH_KEYS is a strict SUPERSET of the fork
                    # list (fork had max_model_len / context_length /
                    # context_window / max_input_tokens /
                    # max_position_embeddings — all present upstream), so no
                    # fork-recognized key is lost.
            ...
            if resp.status_code == 200:
                ...
                if matched is not None:
                    ...
                    #
                    # Parity note (2026-08-29): the fork's own max_tokens
                    # exclusion here is SUBSUMED, not dropped — upstream moved
                    # it into _CONTEXT_LENGTH_KEYS, which is a strict superset
                    # of the fork's hand-rolled tuple (fork's max_input_tokens
                    # and max_position_embeddings are both in it), and
                    # max_tokens lives in _MAX_COMPLETION_KEYS instead.
                    for source in sources:
...
def _split_dense_sparse(text: str) -> tuple[int, int]:
    """Return (cjk_dense_char_count, sparse_char_count) for ``text``.

    ASCII text is the overwhelmingly common case and cannot contain any
    token-dense CJK codepoint, so short-circuit it with a single O(1)
    ``str.isascii()`` flag check (no scan).  Otherwise count dense codepoints
    with one C-level ``re.sub`` rather than a per-character Python loop.
    """
    if not text:
        return 0, 0
    if text.isascii():
    ...
    return dense, len(text) - dense


def estimate_tokens_rough(text: str) -> int:
    """Rough token estimate (~3.5 chars/token, CJK-dense-aware) for pre-flight checks.
    ...
    of applying the chars/token rule.
    ...
    if not text:
        return 0
    text = str(text)
    dense, sparse = _split_dense_sparse(text)
    # Sparse (ASCII / non-CJK) text keeps the historical (len+3)//4 classic
    # rule; token-dense CJK/Hangul/Kana codepoints count ~1 token each.
...
def estimate_messages_tokens_rough(messages: List[Dict[str, Any]]) -> int:
    ...
    image_tokens = 0
    for msg in messages:
        d, sp, img = _estimate_message_tokens_cached(msg, _IMAGE_TOKEN_COST)
        dense_total += d
        sparse_total += sp
        image_tokens += img
    # CJK/Hangul/Kana counted ~1 token each; the sparse (ASCII/non-CJK)
    # remainder uses the tunable ~3.5 chars/token rule, accumulated ONCE across
    # the whole list so multi-message estimates stay additive (ceil applied to
    # the summed char count, not per-message).
    return dense_total + _ceil_chars_to_tokens(sparse_total) + image_tokens
...
_MSG_TOKENS_CACHE: Dict[Any, Tuple[list, Tuple[int, int, int]]] = {}
...
def _estimate_message_tokens_cached(
    msg: Any, image_cost: int
) -> tuple[int, int, int]:
    """Memoized ``(dense_tokens, sparse_chars, image_tokens)`` for one message.

    Parity note (2026-08-07): upstream memoized a single scalar per message.
    The fork's estimator is a CJK dense / sparse-char SPLIT whose sparse
    remainder must be summed across the whole list before ONE ceil() is
    applied (otherwise per-message rounding inflates multi-message totals).
    So the memo caches the tuple, not a scalar — same soundness argument,
    same cache key, but the additive contract survives.
    """
    try:
        pins: list = []
        ...
        d, sp = _estimate_message_dense_sparse(msg)
        return (d, sp, _count_image_tokens(msg, image_cost))
    cached = _MSG_TOKENS_CACHE.get(key)
    if cached is not None:
        return cached[1]
    d, sp = _estimate_message_dense_sparse(msg)
    tokens = (d, sp, _count_image_tokens(msg, image_cost))
...
def _estimate_tools_tokens_rough(tools: List[Dict[str, Any]]) -> int:
    ...
    tokens = _ceil_chars_to_tokens_fixed(total_chars)
