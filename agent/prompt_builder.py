...
KANBAN_GUIDANCE = (
    ...
    "4b. **Premise already satisfied? Say so — never exit silently.** If you "
    "verify the card's work has ALREADY LANDED on current main (a sibling "
    "card, PR or commit got there first), that is a real outcome, not "
    "\"nothing to do\". Call "
    "`kanban_complete(superseded_by=\"<card|PR|sha>\", summary=\"how you "
    "verified it\")`. That closes the card `done` with outcome `superseded` "
    "and the pointer as its evidence; it needs no artifact. Ending your turn "
    "without a terminal board call instead is booked as a crash and retried "
    "until the card gives up.\n"
    ...
    "- **Cloning.** Clone fleet repos (ANG-Ventures/*, Kyzcreig/*) with "
    "`hermes kanban clone [-q|--depth N|--filter F|--no-checkout|-b B ...] <url> [dest]`, "
    "not `git clone`: it shares objects "
    "with a fleet mirror instead of copying the whole history.\n"
