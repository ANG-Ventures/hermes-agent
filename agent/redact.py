...
# "AUTH" (in "AUTHOR") and so falsely match the secret-name pattern above.
...
#
# FORK-PARITY NOTE (2026-08-07 parity merge): upstream independently fixed the
# same ReDoS with POSSESSIVE quantifiers (``[A-Za-z0-9_\-]++`` etc., py3.11+).
# Measured head-to-head on this box (py3.11.15, adversarial dotted run that fails
# at the end) the fork form is a better COMPLEXITY CLASS, not just a better
# constant: fork is linear (2x per input doubling), upstream's possessive form is
# quadratic (4x per doubling) because possessive quantifiers block backtracking but
# do nothing about the per-offset RESTART cost the lookbehinds above eliminate.
#   n=  500/1000/2000/4000 chars -> fork 0.00006/0.00012/0.00024/0.00047 s
#                                -> upstream 0.01121/0.04455/0.17458/0.69633 s
# (~1480x at 4 KB). Keeping the fork pattern; do NOT 'simplify' to the possessive
# form on a future sync without re-running that scaling curve.
...
def redact_sensitive_json(value, **kwargs):
    ...
    masks to ``{"x": "DB_PASSWORD=***`` and no longer parses (t_d59ca5db).
    Redacting each string (keys included) on its own can never touch JSON
    syntax. The serialized path's key-based rule (``"password": "..."``,
    _JSON_FIELD_RE) is kept: a string value under such a key is masked
    whole. ``kwargs`` are passed through to redact_sensitive_text.
    """
