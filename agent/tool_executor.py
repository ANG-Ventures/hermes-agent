...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for tool_call in tool_calls:
        ...
        if getattr(agent, "_in_budget_grace", False) and not is_readonly_grace_tool(function_name):
            ...
            # Same {"error": ...} shape as the other block paths; the distinct
            # budget_grace_block error_type below carries it to telemetry.
            _grace_msg = grace_block_message(function_name)
            block_result = json.dumps({"error": _grace_msg}, ensure_ascii=False)
            _emit_terminal_post_tool_call(
                agent,
                function_name=function_name,
                function_args=function_args,
