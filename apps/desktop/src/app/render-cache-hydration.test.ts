...
describe('computeListDivergence', () => {
  ...
  it('counts adds, removals, and identity-field drift', () => {
    const cached = [s('a'), s('b', { pinned: true }), s('c')]
    const live = [s('a'), s('b', { pinned: false }), s('d')]
    // b drifted (pin), c removed, d added -> 3
...
describe('AC7 — cursor-seam ordering: delta during the hydrate→replace window', () => {
  ...
  it('wholesale replace discards hydrated rows; final list == live snapshot (no dup/drop)', async () => {
    ...
    // 2. a live delta arrives DURING the window (session b was pinned on the
    ...
    const liveSnapshot = [s('a'), s('b', { pinned: true }), s('c')]
    ...
    expect(store.filter(r => r.id === 'b')[0].pinned).toBe(true)
    expect(store.some(r => r.id === 'phantom-from-delta')).toBe(false)
    // and the divergence counter sees exactly the drift (b pin + c add)
