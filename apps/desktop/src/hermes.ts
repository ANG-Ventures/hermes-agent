...
//
// fork parity NOTE (2026-08-07 upstream merge): upstream renamed this
// `setSessionPinned` -> `setSessionPinnedRemote`. The fork's own callers
// (store/layout.ts, use-session-list-actions.ts) still import the original
// name, and upstream's store/session-pin-sync.ts imports the new one, so both
// names are live against the SAME request. Export the alias rather than
// renaming one set of call sites out from under the other.
...
/** Back-compat alias: the fork's call sites import this name. */
export const setSessionPinned = setSessionPinnedRemote
