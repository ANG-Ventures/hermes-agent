...
def _normalize_job_record(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Legacy cron records stored the raw expression as their display; describe it in plain
    # English on read (storage is untouched). A custom display (e.g. the natural phrase the
    # user typed) is kept.
    schedule = normalized.get("schedule")
    if isinstance(schedule, dict) and schedule.get("kind") == "cron":
        expr = _coerce_job_text(schedule.get("expr")).strip()
        shown = {_coerce_job_text(schedule.get("display")).strip(),
                 _coerce_job_text(normalized.get("schedule_display")).strip()}
        if expr and shown <= {"", expr}:
            display = humanize_cron(expr)
