...
from cron.jobs import get_due_jobs, mark_job_run, save_job_output, advance_next_run, get_job, claim_dispatch, heartbeat_run_claim, resolve_job_ref, AmbiguousJobReference
...
def run_job(
    ...
    try:
        ...
        # Reasoning config resolved AFTER provider authentication (so an auth
        # fallback can first replace the primary model with its configured
        # model). Fork feature: a per-job ``reasoning_effort`` override
        # (scheduler_ext.resolve_cron_reasoning_config) wins when it parses to a
        # valid effort; otherwise falls back to the shared config resolver
        # (per-model override > global) against the final model.
        reasoning_config = scheduler_ext.resolve_cron_reasoning_config(
            job, _cfg, str(model)
        )
