...
- **OQ2 — RESOLVED (2026-06-22, via LCM raw-store recovery): the 00:02 leak WAS a subagent send, same RC#1 — there is no separate main-turn seam.** Recovered from Apollo's immutable LCM raw store (`~/.hermes/lcm.db`, never auto-purged) + `agent.log`: (a) the #tdd session has **546 tool-calls but ZERO `send_message`** in the LCM transcript — the send never happened on the main transcript; (b) the LCM store has **zero rows at 07:02 UTC for any session and zero for the home session** — i.e. the send executed in an **isolated context that doesn't persist to LCM** (exactly a `delegate_task` subagent: `skip_memory=True`, isolated session); (c) `agent.log` shows the #tdd turn was running a **16-way `delegate_task` fan-out** continuously through the 00:02:12 `send_message completed`. The `[6a7a77e3]` parent-session tag on that log line is inherited attribution, not main-thread proof. **Conclusion: the recurrence was the subagent seam (RC#1) the whole time; RC#2 (a hypothetical separate main-turn seam) does not exist.** The instrumentation tripwire stays as defense-in-depth, but the fix shipped in #83 closes the actual cause.
