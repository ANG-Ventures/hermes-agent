...
class GatewayConfig:
    ...
    # Host-starvation classification for the missed-probe escalation: when load1 exceeds
    # max(factor * ncpu, a floor) the loop is STARVED, not wedged, and exit 75 would hand the
    # replacement process the same starved host. The watchdog holds and logs instead, for at most
    # liveness_starvation_max_hold_s of continuous starvation with no successful probe.
    ...
    _SCALAR_DICT_FIELDS = (
        ...
        "loop_watchdog_max_strikes", "liveness_starvation_load_factor", "liveness_starvation_max_hold_s",
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            **{name: _coerce_bool(data.get(name), default) for name, default in _TOPLEVEL_BOOL_DEFAULTS.items()},
            ...
            # Zero is out of range for both (a zero factor would declare every host starved).
            liveness_starvation_load_factor=bounded_float(
                "liveness_starvation_load_factor", DEFAULT_LIVENESS_STARVATION_LOAD_FACTOR, 1e-9, 1000.0),
            liveness_starvation_max_hold_s=bounded_float(
                "liveness_starvation_max_hold_s", DEFAULT_LIVENESS_STARVATION_MAX_HOLD_S, 1e-9, 86400.0),
