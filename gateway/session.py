...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        self.on_session_key_conflict = None
        self._loaded = False
    ...
    def _find_session_key_conflict(self, candidate=None):
        from gateway.routing_identity import routing_owner_identity

        owners = {}
        for key, current in self._entries.items():
            entry = candidate if candidate is not None and candidate.session_key == key else current
            for owner in (routing_owner_identity(key, None), routing_owner_identity(key, entry.origin)):
                if owner is None:
                    continue
                previous = owners.get(owner)
                if previous is not None and previous != key:
                    left, right = sorted((previous, key))
                    return left, right
                owners[owner] = key
        return None
    ...
    def _assert_unique_session_routes(self, candidate=None) -> None:
        ...
        message = f"Duplicate session route refused: {conflict[0]} / {conflict[1]}"
        if conflict not in self._reported_session_key_conflicts:
            self._reported_session_key_conflicts.add(conflict)
            logger.error(message)
            if self.on_session_key_conflict is not None:
                try:
                    self.on_session_key_conflict(message)
                except Exception:
        ...
        with self._lock:
            ...
            for key, entry in self._entries.items():
                ...
                if kind not in {"dm", "group", "thread"}:
                    continue
                if entry.origin and entry.origin.prospective_thread_id:
                    kind = "thread"
                parts[3] = kind
                groups.setdefault(":".join(parts), []).append((key, entry))
            merged = 0
            ...
            if _db:
                ...
                if callable(replacer):
                    try:
                        ...
                        # Fork parity: a caller that demanded a durable primary
                        # commit (require_primary=True, e.g. clearing a /model
                        # override) must see the failure, not a silent
                        # JSON-only fallback that reverses after restart.
                        if require_primary:
                            raise
            ...
            if session_key in self._entries and not force_new:
                ...
                if _stale_reason and entry.session_id == _stale_session_id:
                    ...
                    if _reset_reason:
                        ...
                        # conversation.  had_any_turn is the durable signal — set
                        # on the first real turn and never zeroed by transcript
                        # compression. Fall back to last_prompt_tokens > 0 for
                        # entries persisted before had_any_turn existed (older
                        # sessions.json rows default the flag to False but may
                        # still carry a non-zero token count). total_tokens is
                        # never populated (token totals live in the SessionDB),
                        # so the old `total_tokens > 0` gate never fired.
