...
# It is wrong for a loop merely STARVED of CPU, because the replacement process contends for the same
# starved host: runaway CPU burners drove load to 538 on 32 cores, the watchdog exited 75 twice, and the
# supervisor relaunched into a 12-minute boot with every adapter timing out. Both present identically
# at the probe, so the classification is explicit: ``load1 > max(factor * ncpu, floor)`` is starved.
# The absolute floor keeps a 2-4 core host from being declared starved at a load that is merely busy.
DEFAULT_LIVENESS_STARVATION_LOAD_FACTOR = 2.0
LIVENESS_STARVATION_LOAD_FLOOR = 8.0
# A starved hold is bounded: after this much CONTINUOUS starvation with no successful probe, exit 75
# anyway. Past this point the distinction stops mattering; whatever it is, it is not resolving itself.
...
def _sample_host_load() -> tuple[Optional[float], Optional[int]]:
    """``(load1, ncpu)``, either ``None`` when unavailable. Never raises: this runs on a path about to
    hard-exit, so a platform without ``os.getloadavg`` (Windows) reports ``None`` rather than costing
    the diagnostic dump. An unavailable load average classifies as WEDGED: absence of evidence for
...
class _LivenessMissDecision:
    ...
    def __init__(self, *, action: str, strikes: int, phase: Optional[str],
...
def evaluate_liveness_miss(
    *, strikes: int, strikes_limit: int, load1: Optional[float], ncpu: Optional[int],
    ...
    """Classify an escalated missed-probe run as starved (hold) or wedged (exit). Pure, so the decision
    is testable without generating real load; generating load is the incident this guards against."""
    try:
    ...
    if not starved:
        return _LivenessMissDecision(action="exit", strikes=strikes, phase=None, starved_since=None)
    ...
    if now - began >= ceiling:
        return _LivenessMissDecision(
            action="exit", strikes=strikes, phase="liveness_starved_giveup", starved_since=began)
    # Hold: back the counter off to one below the limit so the next HEALTHY probe clears it outright
    # and the next MISSED one re-evaluates, rather than the hold swallowing every future miss.
    return _LivenessMissDecision(
        action="hold", strikes=max(strikes_limit - 1, 0), phase="liveness_starved", starved_since=began)
...
def _format_starvation_line(
    ...
    return f"PHASE={phase} load1={load_txt} ncpu={ncpu_txt} strikes={strikes} held_s={held_s:.1f}"
    ...
    def _watchdog() -> None:
        ...
        while not stop_event.wait(timeout=probe_interval):
            ...
            if decision.action == "hold":
                # The structured line IS the page: this module has no alert channel of its own.
                with contextlib.suppress(Exception):
                    logger.critical(
                        "Gateway event loop missed %d consecutive liveness probes, but the HOST is "
                        "starved, not the loop wedged; HOLDING instead of exiting %d (a restart would "
                        "contend for the same CPU). %s", strikes, exit_code,
                        _format_starvation_line("liveness_starved", load1=load1, ncpu=ncpu,
            ...
            if decision.phase == "liveness_starved_giveup":
                with contextlib.suppress(Exception):
                    logger.critical(
                        "Gateway event loop starvation hold exceeded its ceiling with no successful "
                        "probe; exiting with code %d anyway. %s", exit_code,
                        _format_starvation_line("liveness_starved_giveup", load1=load1, ncpu=ncpu,
                                                strikes=strikes, held_s=held_s))
            else:
                with contextlib.suppress(Exception):
                    logger.critical(
