...
# Manual-compress trigger attribution lives in
# agent.manual_compression_feedback.MANUAL_TRIGGER_CLAUSE — the chokepoint
# summarize_manual_compression appends it to every headline, covering this
# gateway path AND the TUI RPC / TUI slash-mirror / CLI surfaces uniformly.
# Do not re-append it here (double attribution).
...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_undo_command(self, event: MessageEvent) -> str:
        ...
        Rewound rows stay in state.db (active=0) for audit and are hidden from
        re-prompts and search. The cached agent is evicted so the next message
        rebuilds context from the truncated (active-only) transcript.
        """
        source = event.source
        n = 1
        raw_args = event.get_command_args().strip()
        ...
        try:
            # The cache is keyed by the profile-namespaced key; a bare
            # build_session_key(source) misses for every secondary profile.
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show Nous credit balance and the top-up handoff.
