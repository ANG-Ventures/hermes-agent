...
def build_model_options_payload(
    ...
    *,
    ...
    ``apply_picker_prefs`` (fork parity) is off by default so this wrapper's
    upstream consumers keep the full provider set; the fork's desktop/TUI
    picker surfaces pass True to honour the user's ``model.picker`` config —
    see the flag's docs on :func:`build_models_payload`.
    """
    ...
    return build_models_payload(
        ...
        apply_picker_prefs=bool(apply_picker_prefs),
    )
