...
def _needs_interpreter(bin_path: Path) -> bool:
    ...
    if not head.startswith(b"#!"):
        ...
        return False
    shebang_raw = head.decode("utf-8", errors="replace").strip()
    if "python" not in shebang_raw.lower():
    ...
    # Compare case-preserved paths — lowercasing the shebang would falsely
    # mismatch any interpreter path containing uppercase (e.g. /Users on mac).
    exe_dir = str(Path(sys.executable).resolve().parent)
    return exe_dir not in shebang_raw
