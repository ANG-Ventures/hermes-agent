...
def _model_dedup_key(model_id: str) -> str:
    """Case-insensitive dedup key that folds picker-search aliases.
...
def _canonical_model_key(model_id: str, provider: str) -> str:
    ...
    Parity note (2026-08-07): fork's own-slug strip and upstream's search-alias
    fold (``_model_dedup_key``) are ORTHOGONAL dedup axes — compose them so a
    provider-namespaced alias (``kimi/k3`` vs curated ``kimi-k3``) folds too.
    """
    low = str(model_id).lower()
    own_prefix = str(provider).lower() + "/"
    if low.startswith(own_prefix):
        low = low[len(own_prefix):]
    return _model_dedup_key(low)
