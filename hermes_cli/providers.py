...
def resolve_provider_full(
    ...
    #
    #     fork parity (2026-08-07 merge): user-configured custom providers are
    #     resolved FIRST (step 2b below) when they can serve this name. The
    #     bundled ``custom`` plugin profile is a generic stub (empty base_url,
    #     aliases ollama/local/vllm/...) that matches bare "custom" and every
    #     one of those aliases, so consulting plugins first shadowed a real
    #     ``custom_providers:`` entry and returned an unusable ProviderDef with
    #     id="custom" — losing the stable ``custom:<provider_key>`` identity.
    #     Both features are correct; only the ORDER was wrong, so try the
    #     user's own config before the bundled stub and keep the plugin lookup
    #     as the fallback it was written to be.
