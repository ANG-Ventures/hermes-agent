...
# fork-parity: the SessionDB executor-offload seam (bounded heavy-read gate).
# Upstream extracted these handlers from ITS web_server, which never had the
# fork's offload, so the extraction silently reverted it — see the async
# handlers below, which re-graft it onto upstream's (otherwise byte-identical)
# bodies.
_session_db_read = late("_session_db_read")
...
async def search_sessions(
    ...
    # fork-parity: body is upstream's, but the blocking SessionDB work is
    # offloaded off the event loop and routed through the BOUNDED heavy-read
    # gate (fork's ``_read_session_search`` + ``heavy=True`` contract). FTS
    # search is the heaviest read on this surface; without the bound it can
    # saturate the executor and starve every other request.
    def _search():
        db = _open_session_db_for_profile(profile, read_only=True)
        try:
    ...
    try:
        return await _session_db_read(_search, heavy=True)
    except HTTPException:
        raise
    except Exception:
...
async def get_session_stats(profile: Optional[str] = None):
    ...
    # fork-parity: body is upstream's, but the blocking SessionDB work is
    # offloaded off the event loop (fork's ``_read_session_stats`` contract).
    def _read():
        db = _open_session_db_for_profile(profile, read_only=True)
        try:
...
async def get_session_detail(session_id: str, profile: Optional[str] = None):
    # fork-parity: body is upstream's, but the blocking SessionDB work is
    # offloaded off the event loop (fork's ``_read_session_detail`` contract).
    def _read():
        db = _open_session_db_for_profile(profile, read_only=True)
        try:
    ...
    session = await _session_db_read(_read)
...
async def rename_session_endpoint(session_id: str, body: SessionRename):
    ...
    # fork-parity: body is upstream's, but the blocking SessionDB work is
    # offloaded off the event loop (fork's ``_rename_session_record`` contract).
    def _write():
        db = _open_session_db_for_profile(body.profile, read_only=False)
        try:
            sid = db.resolve_session_id(session_id)
    ...
    return await _session_db_read(_write)
