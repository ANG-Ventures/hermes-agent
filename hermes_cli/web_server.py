...
def _read_active_session_count(profile: Optional[str] = None) -> int:
    ...
    # fork parity NOTE (2026-08-07): read_only is keyword-only REQUIRED on the
    # fork's _open_session_db_for_profile; upstream's copy of this helper had no
    # such argument. This is a pure read path, same as every other call site.
...
def _read_session_search(profile: Optional[str], q: str, safe_limit: int):
    ...
    # fork parity NOTE (2026-08-07): see _read_active_session_count -- the fork
    # requires an explicit access mode; this is a read path.
    db = _open_session_db_for_profile(profile, read_only=True)
    try:
