...
class SessionDB:
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 10 and SCHEMA_VERSION == 10:
                ...
                if fts5_available and not _trigram_fts_config_enabled():
                    # Trigram disabled by config: skip the v10 backfill —
                    # the post-migration init drops the table anyway.
                    pass
                elif fts5_available:
            ...
            if current_version < 11:
                ...
                if fts5_available:
                    ...
                    if fts5_available:
                        ...
                        # Skip trigram recreate+backfill entirely when disabled
                        # by config — the post-migration init below drops it
                        # anyway; backfilling first just burns startup time and
                        # WAL space on a multi-GB index we're about to discard.
                        if not _trigram_fts_config_enabled():
                            trigram_ok = False
                        else:
            ...
            if current_version < 18 or self._needs_effective_last_active_backfill(conn):
                ...
                # Column creation is declarative; the data migration remains a
                # re-runnable CTE backfill so stale v1/v2 values repair on open.
                self._backfill_effective_last_active(conn)
                self._record_effective_last_active_backfill(conn)
        ...
        if fts5_available:
            ...
            if self._fts_enabled:
                if not _trigram_fts_config_enabled():
                    # Trigram FTS disabled by config (session_store.trigram_fts:
                    # false). Same degradation path as a trigram-less SQLite:
                    # drop the triggers so message writes don't reference a
                    # table we won't maintain, drop the table itself to free
                    # its (potentially multi-GB) pages, and route CJK search
                    # to the LIKE fallback via _trigram_available=False.
                    self._trigram_available = False
                    ...
                    if base_triggers_need_repair:
                        self._rebuild_fts_indexes(cursor, include_trigram=False)
                elif self._sqlite_supports_trigram(cursor):
                    trigram_enabled = self._ensure_fts_schema(
                        cursor, "messages_fts_trigram", FTS_TRIGRAM_SQL
                    )
                    self._trigram_available = trigram_enabled
                    # Rebuild whenever triggers needed repair — base FTS ALWAYS,
                    # trigram only when it actually came up (include_trigram=
                    # trigram_enabled). The probe can succeed while the real
                    # trigram TABLE creation still fails (trigram_enabled=False);
                    # without rebuilding base FTS here, a trigger-repair would
                    # leave messages_fts un-backfilled and search empty.
                    ...
                    self._trigram_available = False
                    self._drop_trigram_triggers(cursor)
                    # Still rebuild the BASE FTS when triggers needed repair, so a
                    # trigger-repair on a trigram-less SQLite re-populates
                    # messages_fts and search keeps working (upstream contract;
                    # include_trigram=False skips the absent trigram table).
                    if triggers_need_repair:
                        self._rebuild_fts_indexes(cursor, include_trigram=False)
        ...
        def _do(conn):
            ...
            conn.execute(
                """INSERT INTO sessions (
                   ...
                   profile_name, started_at, effective_last_active
                )
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``session_store.trigram_fts: false`` or not yet
