...
_EFFECTIVE_LAST_ACTIVE_BACKFILL_VERSION = "1"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 18 or self._needs_effective_last_active_backfill(conn):
                ...
                # deliberately separate from schema_version so retry builds that
                # already stamped v18 but left stale values get one exact repair.
                # Record journal mode for the WAL-vs-DELETE liveness contract.
                self._backfill_effective_last_active(conn)
                self._record_effective_last_active_backfill(conn)
