...
class SessionDB:
    ...
    def _fts_rebuild_finish(self) -> None:
        ...
        when ``session_store.trigram_fts`` is false (or the SQLite build has
        no trigram tokenizer), ``__init__`` drops ``messages_fts_trigram``,
        so an unconditional INSERT raises ``no such table`` and aborts the
        whole rebuild — taking ``optimize_fts_storage()`` down with it.
        """
            ...
            if self._db_has_legacy_inline_fts(cursor):
                ...
                if self._fts_enabled:
                    if not _trigram_fts_config_enabled():
                        # Trigram FTS disabled by config
                        # (session_store.trigram_fts: false). Drop the triggers
                        # so message writes don't reference a table we won't
                        # maintain, drop the table to free its pages, and route
                        # CJK search to the LIKE fallback.
                        self._trigram_available = False
                ...
                if self._fts_enabled:
                    if not _trigram_fts_config_enabled():
                        # Trigram FTS disabled by config
                        # (session_store.trigram_fts: false). Drop the triggers
                        # + table to free pages and route CJK search to LIKE.
                        self._trigram_available = False
                        self._drop_trigram_triggers(cursor)
                        try:
                            cursor.execute(
        ...
        def _do(conn):
            ...
            conn.execute(
                """INSERT INTO sessions (
                   ...
                   profile_name, git_repo_root, started_at, effective_last_active
                )
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)
