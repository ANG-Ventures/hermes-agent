...
# re-pointed DEFAULT_DB_PATH. On the fork DEFAULT_DB_PATH is a LAZY module
# attribute (__getattr__ above), so there is no real global at import time;
# a test monkeypatch (setattr on the module) materializes a real global that
# shadows the lazy resolution — _default_db_path honors exactly that case.
_IMPORT_DEFAULT_DB_PATH = get_hermes_home() / "state.db"

# Fork-only: version marker for the effective_last_active denormalization
# backfill (the FTS storage-layout constant moved to hermes_state_common).
...
def _default_db_path() -> Path:
    ...
    1. A deliberately re-pointed ``DEFAULT_DB_PATH`` (a REAL module global —
       tests monkeypatch the module attribute, which shadows the lazy
       ``__getattr__`` — differing from the import-time snapshot) wins.
...
# The test-context predicate lives in the LEAF module ``hermes_test_context`` so
# callers can reach it without importing this module's dependency graph (which
# transitively pulls ``agent.redact``, whose enable-toggle is an import-time
# snapshot). Re-exported here: this remains the canonical name for existing
# callers and tests, and there is still exactly ONE definition.
from hermes_test_context import (  # noqa: F401
...
# Base (non-trigram) FTS triggers — the expected trigger set when the trigram
# index is disabled via config (session_store.trigram_fts: false).
_FTS_BASE_TRIGGERS = tuple(
    t for t in _FTS_TRIGGERS if t not in _FTS_TRIGRAM_TRIGGERS
)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def _bump_effective_last_active_for_message(
        ...
        if not root_id:
            return
        # The legacy CTE uses the latest message timestamp when a session has
        # messages, falling back to started_at only for empty rows. A first
    ...
    def audit_effective_last_active(self, limit: int = 100) -> List[Dict[str, Any]]:
        with self._read_ctx() as conn:
            ...
            for row in rows:
                expected = self._expected_effective_last_active(conn, row["id"])
    ...
    def _list_sessions_rich_denorm(
        ...
        *,
        ...
        """Indexed two-stage order_by_last_active path gated by config."""
        where_clauses = ["s.effective_last_active IS NOT NULL"]
        params: List[Any] = []
        if source:
        ...
        return self._finish_session_list_rows(
            rows,
    ...
    def _clone_message_tail_rows(
        ...
        *,
        ...
        """Re-insert existing message rows as fresh active rows (pure SQL clone).

        Column-exact except ``id``/``active``/``compacted`` (and
        ``session_id`` when *retarget* redirects the clone into
        *session_id*): new id, active=1, compacted=0, all other fields exact
        — no decode/re-encode round trip, no field drift. Used by the
        compression rotation and archive paths to re-sequence a concurrent
        tail after the compacted set.

        Denorm contract (test_session_list_denorm_reland): every message
        INSERT path must keep ``effective_last_active`` correct, so this
        always ends with _recompute_effective_last_active_for_session for
        the receiving session.
        """
        if not row_ids:
            return
        placeholders = ",".join("?" for _ in row_ids)
        excluded = ("id", "active", "compacted") + (
            ("session_id",) if retarget else ()
        )
        clone_cols = [
            c for c in self._message_column_names(conn) if c not in excluded
        ]
    ...
    def get_messages_after(
        ...
        with self._read_ctx() as conn:
            cursor = conn.execute(sql, (session_id, since, row_limit))
            rows = cursor.fetchall()
        result = []
        for row in rows:
            msg = dict(row)
    ...
    def _raise_if_rewind_would_orphan_tool(
        self, session_id: str, target_message_id: int, conn=None
    ) -> None:
        """Fail before an id-range rewind can leave a tool row without owner.

        ``conn``: when supplied, the read runs on the caller's live write
        connection. rewind_to_message now calls this from INSIDE
        ``_execute_write``, which already holds ``self._lock``; re-acquiring
        that non-reentrant lock here would deadlock.
        """
        if conn is not None:
            rows = conn.execute(
            ...
            with self._read_ctx() as read_conn:
