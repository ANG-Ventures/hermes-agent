...
class SessionSchemaMixin:
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            self._record_effective_last_active_backfill(self._conn)
        else:
            ...
            if current_version < 16:
                ...
                try:
                    ...
                    pass
            if current_version < 18 or self._needs_effective_last_active_backfill(
                self._conn
            ):
            ...
            if self._fts_stale:
                ...
                if self._fts_enabled:
                    if not _trigram_fts_config_enabled():
                        # Fork: trigram FTS disabled by config
                        ...
                        # CJK search to the LIKE fallback. Re-grafted over
                        # upstream's admission-gated rebuild: the base-trigger
                        # repair still runs, but through
                        # _run_admitted_startup_rebuild so a startup rebuild
                        # cannot stampede (upstream's fts_rebuild_admission).
                        self._trigram_available = False
                ...
                if self._fts_enabled:
                    if not _trigram_fts_config_enabled():
                        ...
                        # Re-grafted over upstream's admission-gated rebuild.
                        self._trigram_available = False
                        self._drop_trigram_triggers(cursor)
                        try:
                            cursor.execute(
