...
_SENSITIVE_PATTERN_CATALOG: dict[str, re.Pattern[str]] = {
    ...
    # Fork-only catalog entries (5). An earlier merge took upstream's SMALLER
    # 4-pattern catalog on the mistaken belief upstream's ingest_protection was a
    # superset — it is not. These bare-token matchers (no key:value prefix
    # required) are what the fork's redaction-corpus and storage-security tests
    # exercise; without them a bare `sk-proj-...` / `op://...` / cookie / AWS /
    # GitHub token flows UNREDACTED into summary_nodes and raw storage.
