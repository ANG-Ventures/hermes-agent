...
def test_transient_dropped_durable_kept(tmp_path, monkeypatch):
    # Redirect the dropped-log into tmp so we don't touch the real inbox.
    log = tmp_path / "_dropped-log.jsonl"
    monkeypatch.setattr(cr, "_DROPPED_LOG", str(log))
    # Filter must be importable for this test to be meaningful.
    assert cr._is_transient is not None, "transient filter failed to import — check ~/gbrain/scripts"
...
def test_toggle_off_keeps_both(tmp_path, monkeypatch):
    monkeypatch.setattr(cr, "_DROPPED_LOG", str(tmp_path / "_dropped-log.jsonl"))
