...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _record_command_sync_rate_limit(self, app_id: Any, fingerprint: str, retry_after: float) -> None:
        ...
        state = self._read_command_sync_state()
        key = self._command_sync_state_key(app_id)
        _prev_raw = state.get(key)
        previous: Dict[str, Any] = _prev_raw if isinstance(_prev_raw, dict) else {}
        ...
        state[key] = {
            **previous,
            ...
            # A reconnect landing inside this window must NOT restart the diff
            # and re-burn Discord's per-app command-management bucket.
    ...
    async def _safe_sync_slash_commands(self) -> Dict[str, int]:
        ...
        for key, desired in desired_by_key.items():
            ...
            if self._patchable_app_command_payload(current_existing_payload) == self._patchable_app_command_payload(desired):
                ...
                # left to restore it. That is the inferred mechanism behind the
                # 2026-09-20 loss of /stop and /restart. Never delete first.
        ...
        # ordered per-command mutations. Delete obsolete commands FIRST to stay
        # under Discord's 100-command limit (error 30032 fails the ENTIRE sync),
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reasoning", description="Show/change reasoning effort: none, minimal, low, medium, high, xhigh, max, ultra")
        @discord.app_commands.describe(effort="Effort: none, minimal, low, medium, high, xhigh, max, or ultra. Or reset/show/hide. Leave empty to see current.")
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if rj is not None:
        seeded_extra["reaction_journal"] = str(rj)
        if not _skip_env_bridge and not os.getenv("DISCORD_REACTION_JOURNAL"):
