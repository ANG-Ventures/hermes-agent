...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _record_rich_sent(chat_id: Any, message_id: Any, content: str) -> None:
        ...
        OFF-LOOP.  ``rich_sent_store.record`` is a read-modify-write ending in
        an ``os.replace``, executed inline on the calling thread.  Both call
        sites are on the send path inside ``async def``, so on the running loop
        this stalls the entire gateway for a filesystem write nothing awaits.
        """
        try:
