...
def test_c1_strips_first_class_provider_prefix_from_recorded_model():
    from plugins.blackbox import _normalize_record_provider_model as norm

    # First-class fN provider composite -> split.
    prov, mdl = norm("", "claude-apx-6/claude-haiku-4-5")
    assert (prov, mdl) == ("claude-apx-6", "claude-haiku-4-5"), (prov, mdl)
    # Provider column already set -> trust it, still strip the model prefix.
    prov, mdl = norm("claude-apx-6", "claude-apx-6/claude-haiku-4-5")
    assert (prov, mdl) == ("claude-apx-6", "claude-haiku-4-5"), (prov, mdl)
...
def test_c1_preserves_aggregator_vendor_prefix_verbatim():
    ...
    # OQ-1: on an aggregator, vendor/model IS the canonical id -> untouched.
    prov, mdl = norm("openrouter", "anthropic/claude-haiku-4.5")
...
def test_c1_leaves_bare_model_untouched():
    ...
    prov, mdl = norm("claude-apx-1", "claude-opus-4-8")
