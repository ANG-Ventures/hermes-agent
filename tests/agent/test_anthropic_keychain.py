...
class TestReadClaudeCodeCredentialsFromKeychain:
    ...
    def test_returns_none_when_security_command_not_found(self):
        ...
        with patch("agent.anthropic_credentials.subprocess.run",
...
class TestReadClaudeCodeCredentialsPriority:
    ...
    def test_keychain_takes_priority_over_json_file(self, tmp_path, monkeypatch):
        ...
        # Mock Keychain to return a "newer" token. Re-assert Darwin: the
        # autouse _block_real_claude_keychain fixture patches the SHARED
        # platform module to "Linux" (parity 2026-08-30 — the fork test
        # carried this re-patch; restored after the credentials split).
        with patch("agent.anthropic_credentials.platform.system", return_value="Darwin"),              patch("agent.anthropic_credentials.subprocess.run") as mock_run:
