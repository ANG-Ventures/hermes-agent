...
class TestPrologueMoaAndInPlaceBackfill:
    ...
    def test_inplace_compaction_backfills_sidecar_into_db(self):
        ...
        agent.context_compressor = types.SimpleNamespace(
            ...
            # Fork P2 "compact on the truth" calibration hooks that
            # turn_context reads directly (not getattr-guarded). Return
            # numeric ints so the fork's ``f"{_calibrated:,}"`` /
            # ``f"{_preflight_tokens:,}"`` skew-log at turn_context.py:938
            # formats (the upstream test doubles otherwise leave MagicMocks
            # there → "unsupported format string passed to MagicMock").
            note_rough_sent=lambda _rough_tokens: None,
            calibrated_tokens=lambda _rough_tokens: _rough_tokens,
            _current_skew=lambda: 1.0,
        )
        # Calibrated trigger delegates to should_compress so the once-then-stop
        # counter above still drives the fork's calibrated preflight decision.
        agent.context_compressor.should_compress_calibrated = (
            lambda _tokens=None: agent.context_compressor.should_compress(_tokens)
        )

        def _compress(messages, _system, approx_tokens=None, task_id=None, **_kwargs):
...
class TestSessionRowExistsBeforePreflightCompaction:
    ...
    def _make_agent(self, db, sid, *, in_place):
        ...
        # Fork P2 calibration hooks: turn_context reads these directly and
        # formats the result with ``:,`` (turn_context.py:938). A bare
        # MagicMock leaves _calibrated / _preflight_tokens as MagicMocks →
        # "unsupported format string passed to MagicMock.__format__". Return
        # numeric ints and delegate the calibrated trigger to should_compress.
        compressor.note_rough_sent = lambda _rough_tokens: None
        compressor.calibrated_tokens = lambda _rough_tokens: _rough_tokens
        compressor._current_skew = lambda: 1.0
        compressor.should_compress_calibrated = (
            lambda _tokens=None: _should_compress(_tokens)
        )
