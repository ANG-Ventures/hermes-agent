...
class TestResolveProviderClientAutoRuntimeModel:
    ...
    def test_auto_provider_does_not_let_stale_config_model_override_runtime_model(self):
        ...
        with patch(
            ...
            # parity 2026-08-30: provider="auto" now routes through
            # _resolve_auto_route (3-tuple: client, model, provider identity).
            "agent.auxiliary_client._resolve_auto_route",
            return_value=(codex_client, "gpt-5.5", "openai-codex"),
        ) as mock_resolve_auto:
            from agent.auxiliary_client import resolve_provider_client
        ...
        # 2026-06-29 upstream merge: _resolve_auto gained a `task` param (threaded
        # by the call site as task=task); the atomic-pair fix this test guards is
        # unchanged. Expect the task kwarg in the call.
        mock_resolve_auto.assert_called_once_with(
            main_runtime={"provider": "openai-codex", "model": "gpt-5.5"}, task=None
        )
        assert model == "gpt-5.5"
        assert model != "claude-opus-4-8"
    ...
    def test_auto_provider_preserves_explicit_model_override(self):
        ...
        with patch(
            # parity 2026-08-30: auto now routes via _resolve_auto_route (3-tuple)
            "agent.auxiliary_client._resolve_auto_route",
            return_value=(codex_client, "gpt-5.5", "openai-codex"),
        ):
            from agent.auxiliary_client import resolve_provider_client
