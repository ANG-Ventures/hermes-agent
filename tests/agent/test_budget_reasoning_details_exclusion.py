...
class TestPreflightExcludesReasoningDetails:
    ...
    def test_preflight_estimate_ignores_reasoning_details(self):
        # Parity note (2026-08-07): upstream's preflight estimator was
        # ``_estimate_message_tokens_without_images`` (a single scalar per
        # message). The fork replaced it with a CJK dense/sparse SPLIT,
        # ``_estimate_message_dense_sparse``, so the sparse remainder can be
        # summed across the whole list before ONE ceil() is applied. Same
        # property under test, current function name.
        from agent.model_metadata import _estimate_message_dense_sparse

        def _estimate(msg):
            dense, sparse = _estimate_message_dense_sparse(msg)
        ...
        plain = _estimate(base)
        with_details = _estimate(loaded)
