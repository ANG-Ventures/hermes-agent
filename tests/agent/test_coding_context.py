...
def _assert_bounded_git_probe_never_takes_optional_index_lock(tmp_path):
    """A timed-out probe must not be able to orphan git's optional index lock.

    The SIGSTOP arm proves the watcher catches the old behavior rather than
    granting a vacuous pass.  The production arm drives ``coding_context._git``
    with the same status command and must never observe the lock at all.
    """
    _git_init(tmp_path)
    ...
    subprocess.run(
        [git, "-C", str(tmp_path), "commit", "-qm", "many files"],
        check=True,
        env={
            **os.environ,
            "GIT_AUTHOR_NAME": "t", "GIT_AUTHOR_EMAIL": "t@t",
    ...
    # Negative control: old behavior creates the lock. Run enough independently
    # stale refreshes that the watcher is not betting on one sub-millisecond
    # scheduling window, and SIGSTOP the first process observed holding it.
    ...
    for attempt in range(1, 9):
        ...
        negative_watcher = threading.Thread(target=stop_on_lock, daemon=True)
        negative_watcher.start()
        gate.touch()
        ...
        proc.wait(timeout=5)
        negative_watcher.join(timeout=1)
        if stopped.is_set():
            break

    assert stopped.is_set(), "negative control never observed git's optional index lock"
    assert old_probe is not None
    if old_probe.poll() is None:
        os.kill(old_probe.pid, signal.SIGKILL)
    old_probe.wait(timeout=5)
    if lock.exists():
        lock.unlink()

    # Production: the same watcher gets eight independently stale refreshes and
    # must never see index.lock, even transiently.
    ...
    production_watcher = threading.Thread(target=watch_for_lock, daemon=True)
    production_watcher.start()
    try:
        ...
        production_watcher.join(timeout=1)
