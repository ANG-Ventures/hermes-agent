...
Scenario: a long-lived session is built with access token T1. The provider's
refresh tokens are single-use (xai-oauth, openai-codex); another process
consumes the refresh token and the single ``device_code`` pool entry now holds
T2 while other sessions are completing calls with T2. T1 gets a 401/403.
Recovery attributes the failure to ``agent.api_key`` (T1), which matches no
entry, so:

* ``try_refresh_matching(api_key_hint=T1)`` returns ``None`` (nothing to
  refresh; correct, refreshing would burn the fresh single-use token), then
* ``mark_exhausted_and_rotate`` hits the single-entry escape and returns
  ``None`` on the premise that handing back the lone entry "does not change
  the credential". That is false here (the entry holds T2), so fallback fires
  while a valid token sits in the pool.
...
class TestXaiSingletonResyncBeforeAdopt:
    ...
    def test_xai_pool_resyncs_from_auth_store_before_adopting(
        ...
        entry = _entry(
            ...
            source="device_code",
            refresh_token="R1",
            last_refresh="2026-01-01T01:00:00Z",
        )
        store = {
