...
class TestFailureAttribution:
    ...
    def _agent(self, pool, failing_key, credential_id=None):
        return SimpleNamespace(
            ...
            _swap_credential=MagicMock(return_value=_SwapOutcome.SWAPPED),
        )
    ...
    def test_unmatched_key_does_not_retry_only_pool_entry(
        ...
        OAuth refresh; live 2026-09-19 xai-oauth incident) — and a second
        unmatched failure stops without quarantining the healthy entry."""
        pool = self._make_pool(
            tmp_path, monkeypatch,
