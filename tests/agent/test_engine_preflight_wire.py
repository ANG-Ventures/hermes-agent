...
def _stub_compressor(*, preflight=None, threshold_tokens=10**9):
    ...
    # Calibrated trigger delegates to should_compress so a test that overrides
    # comp.should_compress post-construction (threshold-path tests) still routes
    # through the fork's calibrated preflight decision.
    comp.should_compress_calibrated = lambda _tokens=None: comp.should_compress(_tokens)
