...
@pytest.mark.parametrize("write_origin,tag", [
    ("background_review", "review"),
    ("side_question", "side_question"),
])
def test_build_cache_parity_fork_tags_the_fork(write_origin, tag):
    from agent.background_review import build_cache_parity_fork

    parent = SimpleNamespace(
        model="grok-4.3", provider="xai-oauth", platform="cli", session_id="parent-sess",
