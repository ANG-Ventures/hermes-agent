...
def test_fallback_timestamp_survives_delayed_sqlite_persistence(
    ...
    def persist_to_sqlite(messages, _conversation_history):
        ...
        agent.persisted_messages = db.get_messages_as_conversation(
            agent.session_id,
            # parity 2026-08-30: fork gates the timestamp surface (cache
            # safety); this upstream test asserts on it, so opt in.
