...
class TestExternalPricingSource:
    ...
    def test_models_dev_entry_builder_extracts_cost_and_cache(self, monkeypatch):
        monkeypatch.setattr(_mdev, "fetch_models_dev", lambda **_kw: _FAKE_MDEV)
    ...
    def test_external_entry_falls_back_to_openrouter_when_models_dev_misses(self, monkeypatch):
        ...
        monkeypatch.setattr(_mdev, "fetch_models_dev", lambda **_kw: {})
        monkeypatch.setattr(
            "agent.usage_pricing.fetch_model_metadata",
...
class TestSubscriptionIncludedNotes:
    ...
    def test_included_cost_has_note(self):
        ...
        # fork contract (fork-permanent, NOTIONAL_OPENROUTER_PROVIDERS):
        # openai-codex lanes are priced NOTIONALLY at catalog rates for cost
        # visibility — status "estimated" with a reconciliation note — instead
        # of upstream's flat subscription_included bash. See the block comment
        # above NOTIONAL_OPENROUTER_PROVIDERS in agent/usage_pricing.py.
        assert result.status == "estimated"
        assert result.amount_usd is not None and result.amount_usd > 0
        assert len(result.notes) > 0
        assert any("estimat" in note.lower() for note in result.notes)
