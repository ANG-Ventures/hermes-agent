...
def test_effective_cwd_ignores_cd_whose_result_the_next_command_does_not_see(tmp_path):
    repo = tmp_path / "repo"
    repo.mkdir()
    # A piped or `||`-chained `cd` does not change where the next command runs.
    assert _effective_cwd(f"cd {repo} | pytest", tmp_path) == tmp_path
    assert _effective_cwd(f"cd {repo} || pytest", tmp_path) == tmp_path
    assert _effective_cwd(f"cd {repo}; pytest", tmp_path) == repo.resolve()
