...
CASES = {
    ...
    # It lives under tests/, so upstream's python_prod lane (product jobs that
    # never import the suite) correctly stays OFF — same rule as the
    # "tests-only → python without python_prod" case below.
    "json fixture → python (fail-open)": (
        ["tests/fixtures/data.json"], _lanes(python=True, python_prod=False)),
