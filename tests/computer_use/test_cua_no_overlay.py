...
class TestEmbeddedDaemonOverlayFlag:
    def test_serve_process_disables_overlay_when_policy_requires_it(self):
        ...
        with patch.object(
            ...
            # parity 2026-08-30: this test asserts the DIRECT spawn shape
            # (command[:2] == [driver, "serve"]), which is the non-darwin
            # branch — upstream's CI is Linux. On a real Mac the darwin
            # branch wraps via CuaDriver.app (TCC identity) and requires the
            # installed app; pin the platform so the asserted contract and
            # the exercised branch agree everywhere.
