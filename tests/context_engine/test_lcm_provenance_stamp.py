...
def _stamp_and_assemble(e, messages, *, leaf_passes, working_messages=None):
    ...
    tail_rows = wm[lac:]
    if leaf_passes == 1 and len(wm) == len(messages):
        tail_rows = [dict(r, **{"_src_idx": lac + o}) for o, r in enumerate(tail_rows)]
...
def test_multipass_does_not_stamp():
    """Multi-pass rebuilds working_messages (len != messages) → no stamp (would mis-index)."""
    e = _engine()
    messages = _msgs(60)
    # simulate a fold: working_messages shorter than messages (leaf_passes>1 OR length mismatch)
    working = messages[:1] + messages[20:]   # a chunk was folded away
...
def test_length_mismatch_does_not_stamp():
    """Even leaf_passes==1, if working_messages length != messages (scaffold drop) → no stamp."""
    e = _engine()
    messages = _msgs(60)
    working = messages[:1] + messages[5:]  # scaffold drop shortened it
    compressed = _stamp_and_assemble(e, messages, leaf_passes=1, working_messages=working)
    kept = [m for m in compressed if m.get("role") != "system" and not m.get("_lcm_summary")]
    assert all("_src_idx" not in m for m in kept), "length-mismatch must NOT stamp"
