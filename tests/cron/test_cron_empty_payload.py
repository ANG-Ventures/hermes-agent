...
def test_tool_update_rejects_the_2026_08_03_destructive_shape(hermes_env):
    ...
    # parity 2026-08-30: the merged tree validates the deliver target BEFORE
    # the empty-payload check, so this destructive shape (blank prompt+script
    # AND bare-platform deliver) is refused by whichever guard fires first.
    # The incident class is blocked either way; atomicity asserted below.
    assert ("nothing to run" in result["error"]) or ("cron Rule #4a" in result["error"])
