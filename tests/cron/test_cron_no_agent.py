...
def _connect_telegram(monkeypatch):
    # parity 2026-08-30: merged scheduler pre-dispatch-validates delivery
    # platforms; read telegram as connected so delivery still fires.
    ...
    # ...and keep the fire-claim heartbeat owned: upstream run_one_job is a
    # thin fire-claim wrapper that re-verifies ownership against the store
    # before running; these jobs were never claimed through tick().
    from cron import scheduler as _sched
    monkeypatch.setattr(_sched, "heartbeat_fire_claim", lambda *_a, **_kw: True)
    # ...and turn OFF upstream's transient-failure page suppression: its
    # "timeout" wording match also catches the script-watchdog timeout these
    # tests stage, and on a recurring job it suppresses the delivery whose
    # CONTENT taxonomy is the entire subject under test. Use the real config
    # toggle (cron.suppress_transient_failure_page) rather than bypassing the
    # gate, so the toggle plumbing stays exercised.
    monkeypatch.setattr(
        _sched,
        "load_config",
        lambda: {"cron": {"suppress_transient_failure_page": False}},
    )
