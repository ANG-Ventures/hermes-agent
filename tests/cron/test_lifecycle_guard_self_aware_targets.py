...
``_HERMES_GATEWAY_LABEL_RE`` matched *any* hermes gateway label, so the Aegis
break-glass profile (launchd job ``ai.hermes.gateway-aegis``) could not run
``launchctl bootout gui/501/ai.hermes.gateway`` to recover a wedged sibling —
which is its entire purpose. The 2026-09-19 state.db corruption recovery had
to be laundered through ``write_file`` + ``ssh localhost 'nohup bash script &'``
as a result (Obsidian note: "state.db Corruption — the launchd ExitTimeOut
Clamp (2026-09-19)").
...
SELF_SYSTEMD = "hermes-gateway-aegis"
...
def systemd_identity(monkeypatch):
    """Pin self-identity to the Aegis systemd unit, as systemd would."""
...
class TestLaunchdSiblingAllowed:
    @pytest.mark.parametrize(
        ...
        [
            ...
            "launchctl disable gui/501/ai.hermes.gateway-apollo",
...
class TestLaunchdSelfBlocked:
    @pytest.mark.parametrize(
        ...
        [
            ...
            "launchctl kickstart -k gui/501/ai.hermes.gateway-aegis",
            "launchctl stop ai.hermes.gateway-aegis",
            "launchctl unload ~/Library/LaunchAgents/ai.hermes.gateway-aegis.plist",
            "launchctl disable gui/501/ai.hermes.gateway-aegis",
    ...
    def test_mixed_in_one_segment_blocked(self, launchd_identity):
        text = "launchctl bootout gui/501/ai.hermes.gateway-aegis ai.hermes.gateway"
        assert contains_gateway_lifecycle_command(text)
...
class TestSystemdSiblingAllowed:
    @pytest.mark.parametrize(
        ...
        [
            ...
            "sudo systemctl restart hermes-gateway-apollo",
...
class TestSystemdSelfBlocked:
    @pytest.mark.parametrize(
        "text",
        [
            "systemctl --user restart hermes-gateway-aegis",
            "systemctl --user stop hermes-gateway-aegis.service",
            "sudo systemctl restart hermes-gateway-aegis",
        ],
    )
...
class TestBootstrapSiblingPlist:
    ...
    def test_self_plist_bootstrap_blocked(self, launchd_identity):
        text = (
            "launchctl bootstrap gui/501 "
            "/Users/ace/Library/LaunchAgents/ai.hermes.gateway-aegis.plist"
        )
...
class TestReferencedScriptInheritsSelfAwareness:
    ...
    def test_sh_c_payload_self_blocked(self, launchd_identity):
        assert contains_gateway_lifecycle_command_or_referenced_script(
            "sh -c 'launchctl bootout gui/501/ai.hermes.gateway-aegis'"
        )
...
class TestNoRegressionOnNonLabelBranches:
    ...
    def test_ssh_remote_exemption_unchanged(self, launchd_identity):
        assert not contains_gateway_lifecycle_command(
            "ssh macbook 'launchctl kickstart -k gui/501/ai.hermes.gateway-aegis'"
        )

    def test_loopback_ssh_self_still_blocked(self, launchd_identity):
        assert contains_gateway_lifecycle_command(
            "ssh localhost 'launchctl bootout gui/501/ai.hermes.gateway-aegis'"
        )
