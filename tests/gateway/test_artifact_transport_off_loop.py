...
``POST /v1/artifacts/upload`` ended in ``ArtifactStore.store``: mkstemp,
write, ``os.fsync``, ``os.replace``.  ``GET /v1/artifacts/download/{id}``
ends in ``ArtifactStore.load``: a whole-file ``read_bytes``, a SHA-256
re-hash, and an ``unlink``.  Both ran inline on the single aiohttp
event-loop thread, so a slow filesystem stalled every other in-flight API
request behind one artifact — the 2026-09-20 incident class.  The upload
half was frozen in ``REACHABLE_BASELINE`` as
``api_server.py _handle_artifact_upload -> os.fsync``; the download half
was the same defect, invisible to the ratchet only because the DFS reports
one sink per coroutine.

These are BEHAVIOURAL tests: they make the filesystem genuinely slow and
assert the loop kept serving, rather than asserting on the shape of the
source.  The store-resolution tests cover the third blocking site on the
same routes — ``ArtifactStore.__init__``'s mkdir + full-directory orphan
sweep plus ``prune_expired`` — which a cold profile pays on its first
artifact request.
"""
