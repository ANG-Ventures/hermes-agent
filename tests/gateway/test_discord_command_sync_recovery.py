...
def adapter(tmp_path, monkeypatch):
    # These tests exercise the default "safe" policy; an inherited operator
    # setting (e.g. DISCORD_COMMAND_SYNC_POLICY=bulk) must not reroute them.
...
async def test_reconnect_inside_backoff_window_skips_the_sync(adapter, monkeypatch, caplog):
    ...
    has not. the bot reconnected 5+ times in one day; each reconnect restarted
