...
async def test_standby_takes_over_without_restart(harness, owner_dies, caplog):
    ...
    if owner_dies:
        process = await asyncio.create_subprocess_exec(
            sys.executable, "-c",
            "from gateway.kanban_watchers_common import _acquire_singleton_lock; "
...
@pytest.mark.asyncio
@pytest.mark.parametrize("work", ["reaper", "dispatch", "auto_decompose", "ready"])
async def test_cancel_does_not_release_while_service_thread_is_running(harness, monkeypatch, work):
    ...
    if work == "auto_decompose":
        h.cfg["kanban"]["auto_decompose"] = True
        target = Mock(return_value=0)
        monkeypatch.setattr(watchers._KanbanDispatcher, "auto_decompose_tick", target)
    elif work == "ready":
        target = Mock(return_value=False)
        monkeypatch.setattr(watchers._KanbanDispatcher, "ready_nonempty", target)
    else:
...
@pytest.mark.asyncio
@pytest.mark.parametrize("unavailable", ["acquisition", "backend_import"])
async def test_unavailable_lock_preserves_config_only_dispatch(harness, monkeypatch, caplog, unavailable):
    h = harness
    b = runner()
    if unavailable == "acquisition":
        monkeypatch.setattr(watchers, "_acquire_singleton_lock", lambda path: (None, "unavailable"))
    else:
        real_import = builtins.__import__

        def without_lock_backend(name, *args, **kwargs):
            if name == "gateway.status":
                raise ImportError("native lock backend unavailable")
            return real_import(name, *args, **kwargs)

        monkeypatch.setattr(builtins, "__import__", without_lock_backend)
    task = asyncio.create_task(b._kanban_dispatcher_watcher())
    try:
        _, resume = await h.clock.paused(task)
        resume.set_result(None)
...
@pytest.mark.asyncio
async def test_repeated_standby_and_leader_shutdown_returns_lock_handles(harness):
    h = harness
    for _ in range(30):
        owner, state = watchers._acquire_singleton_lock(h.path)
        assert state == "held"
