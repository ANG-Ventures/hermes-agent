...
Incident 2026-09-20: two kanban workers ran CPU burners on the production Mac
Studio (load average 538, then 163, on 32 cores). The loop-liveness watchdog
read the resulting probe misses as a wedge and hard-exited 75 twice; launchd
relaunched into a 12-minute boot with all four adapters timing out at once.
Exit-75 exists for a WEDGED loop (deadlock / synchronous block) — a restart
genuinely recovers that. CPU starvation is not a wedge, and a restart makes it
strictly worse, because the new process has to contend for the same CPU.

So the missed-probe path must CLASSIFY before it exits:
  * load1 above max(factor * ncpu, floor) -> ``starved``: log one structured
    line and hold, leaving the strike counter one below the limit so the next
    healthy probe clears it and the next missed one re-evaluates.
  * anything else -> ``wedged``: exit 75, byte-for-byte as before.
A starved hold is bounded: after ``liveness_starvation_max_hold_s`` of
continuous starvation with no successful probe, exit 75 anyway.

Load is always MOCKED here — generating real load is the incident.
"""
...
def test_runner_guards_pass_the_starvation_knobs_to_the_watchdog():
    """Unwired knobs would leave a starved gateway on the exit-75 wedge path."""
    ...
    runner = SimpleNamespace(
        config=GatewayConfig.from_dict(
            {"liveness_starvation_load_factor": 3, "liveness_starvation_max_hold_s": 120}
        ),
        _loop_floor_timer_handle=object(),
        _loop_liveness_watchdog=None,
    )
    with patch("gateway.shutdown_watchdog.start_loop_liveness_watchdog") as start:
        GatewayStartupMixin._start_loop_liveness_guards(runner, MagicMock())
    kwargs = start.call_args.kwargs
    assert kwargs["starvation_load_factor"] == 3.0
    assert kwargs["starvation_max_hold_s"] == 120.0
