...
class TestUserFacingQueueDepth:
    """The ``(N queued)`` count must reflect what the USER queued.

    Synthetic turns share the FIFO with real user messages: kanban
    completion wakes and auto-resume continuations arrive as
    ``MessageEvent(internal=True)``, and ``/goal`` continuations arrive
    as ordinary user-role events with a synthetic prefix. Both occupy
    queue slots (correctly — they must not interrupt a running turn),
    but neither is something the user sent.

    Regression: a user issuing ONE ``/queue`` into a session that already
    had a kanban wake parked was told ``Queued for the next turn. (2
    queued)``. ``_queue_depth`` (resource accounting, backs the pending
    cap) counted both; ``_user_queue_depth`` (user-facing) counts only
    real user messages.
    """
    ...
    def test_internal_wake_is_not_counted_as_a_user_queued_item(self):
        """The reported bug: 1 kanban wake + 1 /queue announced "(2 queued)"."""
        runner, adapter = self._make_runner_and_adapter()
        session_key = "telegram:user:wake"

        # A kanban completion wake lands first (internal=True).
        runner._enqueue_fifo(
            session_key, self._event("[kanban] task done", internal=True), adapter
        )
        # Then the user sends exactly one /queue.
        runner._enqueue_fifo(session_key, self._event("do the thing"), adapter)

        # Resource accounting still sees both — the cap must not be fooled.
        assert runner._queue_depth(session_key, adapter=adapter) == 2
        # But the user only queued one thing.
    ...
    def test_internal_event_in_the_head_slot_is_excluded(self):
        """The synthetic event can occupy the slot OR the overflow."""
        runner, adapter = self._make_runner_and_adapter()
        session_key = "telegram:user:slot"

        runner._enqueue_fifo(
            session_key, self._event("[kanban] wake", internal=True), adapter
        )
