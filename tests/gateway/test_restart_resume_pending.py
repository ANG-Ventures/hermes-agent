...
async def test_background_resume_result_logs_late_failure_not_success_or_cancel():
    ...
    try:
        ...
        pass
    # 2026-08 parity merge: the callback now routes through the shared
    # _log_late_background_failure helper, which emits via
    # logger.log(logging.DEBUG, ...) rather than logger.debug(...).
    import logging as _logging

    with patch("gateway.run.logger") as mock_logger:
        GatewayRunner._log_background_resume_result(failed)
        assert mock_logger.log.call_count == 1
        assert mock_logger.log.call_args.args[0] == _logging.DEBUG
