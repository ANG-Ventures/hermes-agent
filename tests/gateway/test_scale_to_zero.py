...
def _fake_flaps(tmp_path, status_line, capture):
    ...
    # AF_UNIX paths are capped (~104 bytes on macOS/BSD); pytest tmp_path can
    # exceed that. Bind in a short mkdtemp under the system tmp dir instead —
    # tmp_path is kept in the signature so call sites stay uniform.
    import tempfile

    sock_dir = tempfile.mkdtemp(prefix="flaps-")
    sock_path = str(os.path.join(sock_dir, "fly-api.sock"))
