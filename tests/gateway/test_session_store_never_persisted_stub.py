...
def _seed_routes(store: SessionStore, entries: list[SessionEntry]) -> None:
    ...
    for entry in entries:
        ...
        store._routing_db.save_gateway_routing_entry(
            entry.session_key,
            json.dumps(payload),
...
def test_startup_reaps_only_old_inert_missing_rows_and_roundtrips_both_stores(
    ...
    assert {entry.session_key for entry in startup.list_sessions()} == expected_keys
    assert set(
        startup._routing_db.load_gateway_routing_entries(scope=startup._routing_scope())
    ) == expected_keys
    assert (
    ...
    assert {entry.session_key for entry in fresh.list_sessions()} == expected_keys
    fresh._save_entries()
    assert set(
        fresh._routing_db.load_gateway_routing_entries(scope=fresh._routing_scope())
    ) == expected_keys
    assert (
        set(json.loads((sessions_dir / "sessions.json").read_text(encoding="utf-8")))
        - {"_README"}
...
def test_runtime_reaps_inert_missing_row_after_grace_without_restart(
    ...
    later = route.created_at + timedelta(days=2)
    with patch("gateway.session._now", return_value=later), patch(
        "gateway.session_lifecycle._now", return_value=later
    ):
    ...
    persisted = store._routing_db.load_gateway_routing_entries(scope=store._routing_scope())
