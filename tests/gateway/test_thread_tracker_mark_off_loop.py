...
filesystem pressure -- the exact tail that held the Apollo MainThread for >= 30s
on 2026-09-20, several plain-``def`` frames below a coroutine.
...
The reachability ratchet froze this route as
    ...
    plugins/platforms/matrix/adapter.py _resolve_message_context -> atomic_json_write
      via _resolve_message_context>mark>_save>atomic_json_write

``_resolve_message_context`` is on the inbound-message path for every Matrix
text and media event.  Discord reaches the SAME blocking body from
``_handle_message`` and ``_handle_thread_create_slash``; those coroutines were
merely shadowed by a different first sink in the DFS (which reports only the
first sink per coroutine), so they are not separate bugs -- they are the same
one, which is why the fix is at the tracker rather than at the Matrix site.

The fix is therefore a CHOKE-POINT one: ``mark_async`` offloads ``_save`` to a
worker thread, and all eight coroutine call sites await it.  ``mark`` keeps its
exact synchronous contract for the non-loop callers and tests.
...
def test_no_coroutine_calls_the_blocking_thread_mark():
    ...
    flagged here.  That transitive class is the job of the reachability ratchet
    in ``tests/gateway/test_no_atomic_write_reachable_from_loop.py``, which
    walks the call graph; the two gates are complementary and neither subsumes
    the other.
    """
    repo = _repo_root()
    hits: list[str] = []
...
def test_the_sweep_is_not_vacuous():
    ...
    # Measured on this tree: 5 in the Matrix adapter, 3 in the Discord adapter.
    assert len(async_call_sites) >= 8, (
        "the sweep found fewer _threads.mark_async call sites than the 8 this "
