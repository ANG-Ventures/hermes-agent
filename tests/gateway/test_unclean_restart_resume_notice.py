...
Incident 2026-09-20 16:01 (Apollo). The event loop was blocked ~30s writing
``sessions.json`` (``utils.py:230`` ``atomic_replace``); the loop-liveness
watchdog fired ``os._exit(75)`` at 16:01:08 from its own thread. launchd
relaunched the gateway at 16:04:16 and three Discord sessions were boot-resumed
(``PHASE=boot_resume_scheduled ... reason=restart_interrupted``). Their replies
landed at 16:08-16:09 with **no explanation of the seven-minute silence**.
...
def _resume_runner(tmp_path, sentinel, site=None):
    ...
    runner._startup_resume_modes = {}
    runner._unclean_restart_home = tmp_path
    runner._unclean_restart_sentinel = sentinel
    runner._unclean_restart_site = site
