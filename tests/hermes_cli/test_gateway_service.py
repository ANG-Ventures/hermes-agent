...
class TestGeneratedSystemdUnits:
    def _expected_timeout_stop_sec(
        self,
        drain_timeout: float = DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT,
        cron_drain_timeout: float = DEFAULT_GATEWAY_CRON_DRAIN_TIMEOUT,
    ) -> str:
        # Parity 2026-08-29: upstream's helper hard-read
        # DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT, which is 0 upstream but 180 on
        # the fork (config_defaults.py, "fork parity 2026-08-07"). Callers that
        # monkeypatch the effective drain must be able to say so, or the
        # assertion silently compares the unit against a drain nobody
        # configured.
    ...
    def test_timeout_stop_sec_covers_default_cron_drain_floor(self, monkeypatch):
        ...
        assert expected > 60
        assert (
            self._expected_timeout_stop_sec(
                0.0, DEFAULT_GATEWAY_CRON_DRAIN_TIMEOUT
            )
            in unit
        )
