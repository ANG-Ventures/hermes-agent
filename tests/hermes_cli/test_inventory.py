...
def test_excluded_providers_filters_rows_injected_by_inventory():
    ...
    (web_server /api/models, kanban dashboard, ACP adapter, tui_gateway,
    moa_cmd). Both injectors are asserted, plus a positive control.
    """
    from hermes_cli.models import CANONICAL_PROVIDERS
    ...
    skeleton_slug = next(
        e.slug for e in CANONICAL_PROVIDERS if e.slug != "openrouter"
    )
    rows = [
        {"slug": "openrouter", "name": "OpenRouter", "models": ["m1"],
         "total_models": 1, "is_current": True, "is_user_defined": False,
    ...
    ctx = ConfigContext(
        ...
        user_providers={},
        custom_providers=[],
        excluded_providers=["moa", skeleton_slug],
    )

    # Sanity: without exclusions both injected rows are present.
    baseline_ctx = ConfigContext(
        current_provider="openrouter",
        current_model="m1",
        current_base_url="",
        user_providers={},
    ...
    with _list_auth_returning(rows):
        baseline = {
            ...
            for r in build_models_payload(
                baseline_ctx, include_unconfigured=True
            )["providers"]
        }
    assert "moa" in baseline, "sanity: virtual moa row must be injected"
    assert skeleton_slug in baseline, (
    ...
    assert not leaked, (
        f"excluded_providers must hide inventory-injected rows, leaked: {leaked}"
    )
