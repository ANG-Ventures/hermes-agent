...
* ``triage_resolve_task``'s refusal on a ``blocked`` card listed three verbs
...
# ---------------------------------------------------------------------------
# (c) triage-resolve names the verb that actually applies
# ---------------------------------------------------------------------------


def test_triage_resolve_refusal_on_blocked_names_unblock(kanban_home: Path) -> None:
    with kb.connect_closing() as conn:
        tid = _review_task(conn)
        kb.block_task(conn, tid, reason="parked", kind="capability")
        ok, err = kb.triage_resolve_task(
            conn, tid, to="done", reason="approved out of band",
        )
        assert ok is False
        assert err is not None
        assert "hermes kanban unblock" in err
        # It must not offer the verbs that do NOT apply to a blocked card.
        assert "promote" not in err
