...
def test_rewritten_mirror_over_cap_uses_live_tree_instead(board, tmp_path, monkeypatch):
    """The recovery bundle exceeds the cap; a bound live ref fits."""
    import hermes_cli.kanban_survivor as survivor_mod

    tid, ws, live, head, _ = home_clone(board, tmp_path)
    bundle = survivor_mod._snapshot(ws, None, "")
    assert len(bundle) > 750, "fixture must exceed the limit before asserting the escape"
    monkeypatch.setattr(kb, "KANBAN_ATTACHMENT_MAX_BYTES", 750)

    assert kb.complete_task(board, tid, metadata={"changed_files": ["code.py"]})
    recorded = kb.latest_run(board, tid).metadata["survivor"]
    assert recorded["kind"] == "ref"
    assert recorded["refs"][0]["repository_path"] == str(live.resolve())
    assert recorded["refs"][0]["sha"] == head
    assert not ws.exists()
