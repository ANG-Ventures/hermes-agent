...
def test_predicate_delegates_and_is_not_a_second_private_copy(monkeypatch):
    ...
    A source-text or identity assertion would only grade the wiring's appearance. Patching
    the single definition and observing managed_scope's answer change proves the delegation
    is live — a private copy would be unaffected by the patch.
    """
    import hermes_state
    ...
    # ...and it binds the leaf module's definition, which hermes_state re-exports —
    # one definition reached by two names, not two implementations.
    monkeypatch.undo()
    assert managed_scope._in_test_context is hermes_test_context._in_test_context
    assert hermes_state._in_test_context is hermes_test_context._in_test_context


def test_test_context_module_is_a_leaf(tmp_path):
    """The predicate module must not drag in a dependency graph.

    ``hermes_state`` transitively imports ``agent.redact``, which snapshots its
    enable-toggle at import time; reaching the predicate through ``hermes_state`` froze
    that toggle before the config bridge ran (caught by
    tests/hermes_cli/test_redact_config_bridge.py). Importing the leaf must pull neither.
    """
    import subprocess
    import sys

    probe = (
        "import sys; import hermes_test_context; "
...
def test_system_scope_ignored_at_real_interpreter_shutdown(tmp_path):
    ...
    (tmp_path / "test_inner_atexit.py").write_text(
        ...
        "        'ancestor': __import__('hermes_test_context')._has_pytest_ancestor(),\n"
...
def test_self_check_matches_launcher_position_not_any_pytest_token(monkeypatch):
    ...
    def _self(argv, cmdline):
        monkeypatch.setattr(hermes_test_context, "_PYTEST_SELF", None)
        monkeypatch.setattr(hermes_test_context.sys, "argv", argv)
        monkeypatch.setattr(
            hermes_test_context, "psutil",
            type("P", (), {"Process": staticmethod(lambda: type("Q", (), {"cmdline": staticmethod(lambda: cmdline)})())}),
        )
        return hermes_test_context._is_pytest_self()
