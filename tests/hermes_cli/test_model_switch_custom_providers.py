...
def test_excluded_providers_hides_user_defined_rows(monkeypatch):
    ...
    The per-section gates only covered the built-in rows (sections 1, 2, 2b).
    Sections 3 (``providers:``), 3b (bare ``provider: custom`` + ``base_url``)
    and 4 (``custom_providers:``) had no exclusion gate, so an excluded user
    endpoint still reached every non-CLI consumer (web_server /api/models, the
    kanban dashboard, the ACP adapter, tui_gateway, moa_cmd) while
    ...
    custom_providers = [
        ...
        {
            "name": "cp-kept",
            "base_url": "http://127.0.0.1:6/v1",
            "api_key": "x",
            "model": "m6",
            "discover_models": False,
        },
    ]
    # section 3b: the active bare custom endpoint, excluded as ``custom``
    def _rows(excluded):
        return {
            p["slug"]
    ...
    filtered = _rows(
        ["excluded-slot", "Excluded By Name", "cp-excluded", "custom"]
    )
...
def test_excluded_providers_hides_moa_row_from_picker(monkeypatch):
    ...
    assert "moa" in baseline, "sanity: moa row must be injected"
