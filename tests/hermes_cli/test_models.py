...
class TestLocalOllamaModelDiscovery:
    ...
    def test_switch_model_direct_ollama_alias_preserves_matching_origin_api_key(self):
        ...
        try:
            with patch.object(model_switch, "get_model_info", return_value=None), patch.object(
                # Fork's _ensure_direct_aliases hot-reloads from config on every
                # call, which would prune the alias injected above — pin it.
                model_switch, "_ensure_direct_aliases", lambda: None
            ), patch(
    ...
    def test_switch_model_direct_ollama_alias_clears_different_origin_api_key(self):
        ...
        try:
            with patch.object(model_switch, "get_model_info", return_value=None), patch.object(
                # Pin against the fork's hot-reload (see sibling test above).
                model_switch, "_ensure_direct_aliases", lambda: None
            ), patch(
                "hermes_cli.models.validate_requested_model",
                return_value={"accepted": True, "persist": True, "recognized": True, "message": ""},
