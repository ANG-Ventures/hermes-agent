...
def test_enforcement_site_does_not_reference_the_frozen_constant():
    ...
    src = inspect.getsource(tt._plan_execution)
...
def test_schema_quotes_the_enforced_cap(monkeypatch):
    """The model reads the cap from the schema; it must match what is enforced."""
    import tools.terminal_tool as tt
    from tools.registry import registry

    # Import-time snapshot 600 (the static schema's number); the bridge later exports 3600.
    monkeypatch.setattr(tt, "FOREGROUND_MAX_TIMEOUT", 600)
    monkeypatch.setattr(tt, "_terminal_config_bridge_attempted", True)
    monkeypatch.setenv("TERMINAL_MAX_FOREGROUND_TIMEOUT", "3600")
    static_before = tt.TERMINAL_SCHEMA["parameters"]["properties"]["timeout"]["description"]
    overrides = registry.get_entry("terminal").dynamic_schema_overrides()
    desc = overrides["parameters"]["properties"]["timeout"]["description"]
    assert "foreground max: 3600" in desc and "above 3600s" in desc
    assert tt._foreground_max_timeout() == 3600
    # the static schema object is not mutated
    assert tt.TERMINAL_SCHEMA["parameters"]["properties"]["timeout"]["description"] == static_before

    # No drift -> no override (the static schema already quotes the cap).
    monkeypatch.setenv("TERMINAL_MAX_FOREGROUND_TIMEOUT", "600")
    assert registry.get_entry("terminal").dynamic_schema_overrides() == {}


def test_disk_warning_uses_the_bridged_threshold(monkeypatch, tmp_path):
    import tools.terminal_tool_lifecycle as lc

    scratch = tmp_path / "hermes-x"
    scratch.mkdir()
    (scratch / "f").write_bytes(b"x" * 2048)
    monkeypatch.setattr(lc, "_scratch_paths", lambda: [str(scratch)])
    monkeypatch.setitem(lc._disk_usage_cache, "timestamp", 0.0)
    monkeypatch.setenv("TERMINAL_DISK_WARNING_GB", "0.000001")
    assert lc._check_disk_usage_warning() is True
