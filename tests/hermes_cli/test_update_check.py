...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    assert result == 5
    # 2026-06-29 upstream merge: the path now does BOTH fork's HEAD cache-key probe AND
    # upstream's --is-shallow-repository probe, so: HEAD cache-key + origin probe +
    # is-shallow probe + git fetch + git rev-list = 5.
