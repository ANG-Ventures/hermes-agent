...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        # Hermetic: on a real Mac the belt-and-suspenders `launchctl list`
        # prefix scan would pull live host gateway PIDs into the result.
        monkeypatch.setattr(
            gw.subprocess,
            "run",
            lambda *a, **k: (_ for _ in ()).throw(FileNotFoundError("launchctl")),
        )
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
...
class TestIncompleteWarningMentionsLaunchctl:
    def test_launchd_labels_get_launchctl_hint(self, capsys, monkeypatch):
        # The warning branches on the HOST platform; these hints are the
        # non-macOS (systemd-capable) rendering — pin it so the test is
        # host-independent (platform passed as data via the mock).
