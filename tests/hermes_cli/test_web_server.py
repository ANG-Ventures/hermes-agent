...
class TestWebServerEndpoints:
    ...
    def test_get_sessions_heals_stale_schema_store(self, missing_column):
        ...
        try:
            # fork-parity: the fork's DEFERRED_INDEX_SQL adds two indexes that
            # reference `archived` (idx_sessions_{,source_}effective_last_active);
            # upstream's schema has no such reference, so its bare DROP COLUMN
            # dies in SETUP here ("error in index ... no such column") and the
            # heal under test never runs. A real legacy store predating the
            # column could not have carried an index over it — drop the
            # dependents first, exactly as the fork's own
            # test_schema_rollback_drops_indexes_before_column does. The heal
            # path restores BOTH the column and the indexes (verified).
            legacy.execute("DROP INDEX IF EXISTS idx_sessions_source_effective_last_active")
            legacy.execute("DROP INDEX IF EXISTS idx_sessions_effective_last_active")
        ...
        try:
            ...
            indexes = {
                row[1] for row in healed.execute("PRAGMA index_list(sessions)")
            }
        finally:
            healed.close()
        assert missing_column in columns
        # fork-parity: the heal must also rebuild the fork-only recency indexes
        # it just dropped, or session-list ordering silently loses its covering
        # index after any such heal.
        assert "idx_sessions_effective_last_active" in indexes
        assert "idx_sessions_source_effective_last_active" in indexes
