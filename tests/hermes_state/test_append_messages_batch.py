...
class TestAppendMessagesBatch:
    ...
    def test_atomicity_all_or_nothing(self, db, monkeypatch):
        ...
        # fork-parity: the merge's reconciled _insert_message_rows threads
        # ``row_ids_out`` (run_agent needs each committed row id to stamp
        # interrupt-close markers). This upstream-authored stub predates that
        # parameter, so forward it through rather than dropping it — swallowing
        # it here would hide a real signature drift instead of exercising it.
        def failing_insert(
            self_db, conn, session_id, messages, row_ids_out=None
        ):
            ...
            try:
                return real_insert(
                    self_db, conn, session_id, messages, row_ids_out=row_ids_out
                )
            finally:
