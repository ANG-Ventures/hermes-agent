...
class TestEffectiveLastActiveSchema:
    ...
    def test_v18_database_without_backfill_marker_repairs_stale_values_on_open(self, tmp_path):
        db_path = tmp_path / "state.db"
        db = SessionDB(db_path=db_path)
        try:
            _create_session(db, "stale", started_at=10.0)
            _append(db, "stale", "old stored value", 100.0)
            _create_session(db, "fresh", started_at=20.0)
            _append(db, "fresh", "should rank first", 200.0)
        finally:
            db.close()
        ...
        try:
            ...
            conn.execute("UPDATE schema_version SET version = ?", (hermes_state.SCHEMA_VERSION,))
            conn.commit()
        finally:
            conn.close()
        ...
        db = SessionDB(db_path=db_path)
        try:
            assert _stored(db, "fresh") == 200.0
            assert _ordered_ids(db, limit=2) == ["fresh", "stale"]
...
def test_real_copy_top400_matches_cte_oracle_when_requested(tmp_path):
    source = os.environ.get("SESSION_LIST_REAL_COPY_DB")
    if not source:
        pytest.skip("set SESSION_LIST_REAL_COPY_DB to a static state.db copy")

    source_path = Path(source)
    if not source_path.exists():
        pytest.fail(f"real-copy state DB does not exist: {source_path}")
    ...
    for suffix in ("-wal", "-shm"):
        ...
        if sidecar.exists():
            shutil.copy2(sidecar, Path(f"{db_path}{suffix}"))

    conn = sqlite3.connect(db_path)
    try:
        columns = {row[1] for row in conn.execute("PRAGMA table_info(sessions)")}
        if "effective_last_active" in columns:
            conn.execute(
                "DELETE FROM state_meta WHERE key = ?",
                (hermes_state._EFFECTIVE_LAST_ACTIVE_BACKFILL_META_KEY,),
            )
            row = conn.execute(
                "SELECT id FROM sessions "
                "WHERE effective_last_active IS NOT NULL "
                "ORDER BY effective_last_active DESC, started_at DESC, id DESC "
                "LIMIT 1"
            ).fetchone()
            if row is not None:
    ...
    try:
        ...
        seed = db.list_sessions_rich_cte_oracle(limit=1)
        if seed:
            variants.append(("id_query", {"id_query": seed[0]["id"][:8]}))

        for label, kwargs in variants:
            denorm = db.list_sessions_rich(
                limit=400,
                order_by_last_active=True,
                **kwargs,
            )
            oracle = db.list_sessions_rich_cte_oracle(limit=400, **kwargs)
            assert denorm == oracle, label

        assert db.audit_effective_last_active(limit=2000) == []
