...
class TestCanonicalCostExport:
    ...
    def test_included_route_does_not_pin_total(self, monkeypatch):
        ...
        # Fork routing prices openai-codex notionally via OpenRouter for cost
        # visibility (NOTIONAL_OPENROUTER_PROVIDERS), so no real provider
        # resolves to subscription_included here. Pin the route directly: the
        # contract under test is the langfuse EXPORT policy for
        # subscription-included routes, not the fork's routing table.
        monkeypatch.setattr(
            pricing,
            "resolve_billing_route",
            lambda *a, **kw: pricing.BillingRoute(
