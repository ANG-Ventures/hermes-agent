"""FORK CANARY (t_39f3a794): importing a module that model-provider plugins import must not run discovery.

Fork main made this lazy in e8249c38cb (#816): ``_LazyProviderPrefixes`` / ``_LazyUrlToProvider`` in
``agent/model_metadata.py`` and ``_LazyCanonicalProviders`` / ``_LazyProviderLabels`` /
``_LazyKnownProviderNames`` in ``hermes_cli/models.py``. Upstream kept the eager forms (d143bf7a3b and the
``sync_plugin_provider_catalog()`` call in ``models_catalog_static``), and the 2026-09 parity tree took them:
a process whose first import was ``hermes_cli.models`` then registered 0 of the fleet's 52 box providers
(every plugin failed with a circular ImportError). A parity merge that resolves either file toward the
eager upstream form turns this test red. Upstream fix: NousResearch/hermes-agent#122138.

Each case runs in a fresh interpreter, because import order is the bug.
"""
...
@pytest.mark.parametrize("entry", ["hermes_cli.models", "agent.model_metadata"])
