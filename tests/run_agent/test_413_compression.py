...
class TestPreflightCompression:
    ...
    def test_compress_context_emits_lifecycle_status_before_work(self, agent):
        ...
        # The built-in engine emits its fork-owned in-chat compaction announce
        # (the "Context compacted: …" lifecycle line) between the started edge
        # and the structured terminal edge. The announce is a fork feature
        # (agent/fork_ext/compaction_ext.py) delivered via _emit_status; the
        # engine did NOT opt out of automatic status here, so it fires.
        assert events[0] == ("lifecycle", COMPACTION_STATUS)
        assert events[1] == ("compress", "started")
        assert events[-1] == ("compacted", COMPACTION_DONE_STATUS)
        assert any(
            ev == "lifecycle" and "Context compacted" in msg for ev, msg in events
        )
    ...
    def test_pre_api_compression_status_suppressed_when_engine_opts_out(self, agent):
        ...
        # Seed a measured skew of 1.0 so the fork's cold-start trigger prior
        # (_trigger_skew ~0.7 on empty history) does not scale the rough estimate
        # below threshold and defer this preflight case (calibrated-trigger delta).
        agent.context_compressor._recent_skews = [1.0]
        agent.context_compressor.emit_automatic_compaction_status = False
    ...
    def test_pre_api_compression_status_emitted_by_default(self, agent):
        ...
        agent.context_compressor.context_length = 200_000
        agent.context_compressor.threshold_tokens = 130_000
        # Seed a calibrated (measured) skew of 1.0 so the fork's cold-start
        # trigger prior (_trigger_skew ~0.7 on empty history) does not scale the
        # 144,669 estimate below the 130,000 threshold and defer this control
        # case. A live session reaches this state after one paired real reading.
        agent.context_compressor._recent_skews = [1.0]
