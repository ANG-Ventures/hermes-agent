...
class TestRunConversationRecoversFromCorruptImage400:
    def test_strip_and_retry_succeeds_at_the_sequenced_provider_layer(self):
        ...
        # Fork seam (parity 2026-08-29): the fork expires PRIOR-TURN image
        # payloads to text projections at the turn boundary
        # (_degrade_prior_turn_multimodal_messages — fork-only; upstream
        # replays history images verbatim, which is where this corrupt-replay
        # bug lived). So no attempt ever carries an image_url from history;
        # the projection placeholder rides in its place, and the recovery
        # branch still completes the turn on retry.
        first_msgs = calls[0]
        assert not any(
            isinstance(m.get("content"), list)
            and any(p.get("type") == "image_url" for p in m["content"])
            for m in first_msgs
        ), f"prior-turn image should be degraded to text, got: {first_msgs!r}"
        assert any(
            isinstance(m.get("content"), str) and "[screenshot]" in m["content"]
            for m in first_msgs
        ), f"expected [screenshot] projection in first attempt, got: {first_msgs!r}"

        # Retry carries no image parts either.
        second_msgs = calls[1]
        assert not any(
            isinstance(m.get("content"), list)
            and any(p.get("type") == "image_url" for p in m["content"])
    ...
    def test_unrelated_400_with_image_parts_does_not_strip_or_retry(self):
        ...
        # REGRESSION GUARD: the unrelated 400 caused no strip-and-retry. On
        # the fork the prior-turn image was already expired to a text
        # projection at the turn boundary (fork seam — see the positive test
        # above), so assert the projection survived intact instead of a raw
        # image_url part; a reintroduced generic fallback would still fail
        # the single-call assertion above by retrying.
        only_call_msgs = calls[0]
        assert any(
            isinstance(m.get("content"), str) and "[screenshot]" in m["content"]
            for m in only_call_msgs
        ), (
            "expected the degraded [screenshot] projection to survive the "
            f"unrelated 400 untouched: {only_call_msgs!r}"
        )
