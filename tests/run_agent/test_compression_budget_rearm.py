...
def test_pre_api_compression_budget_rearms_only_after_pressure_clears(
    ...
    # Fork API: the preflight prefers should_compress_calibrated (skew-
    # calibrated rough estimate) over raw should_compress. A bare MagicMock
    # auto-attribute would be truthy on every call and force spurious
    # compactions, so mirror the same threshold contract here.
    compressor.should_compress_calibrated.side_effect = (
        lambda tokens, messages=None: tokens >= 100
    )
