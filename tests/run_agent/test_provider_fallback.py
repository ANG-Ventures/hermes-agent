...
class TestFallbackChainAdvancement:
    ...
    def test_records_user_visible_switch_with_reason(self):
        # Fork contract (2026-08-07 duplicate-alert fix): the hop is announced
        # once via _emit_fallback_announce (see tests/agent/test_fallback_announce.py);
        # no second _pending_fallback_notice line and no generic buffered status
        # line may be staged.  Durable provenance for the primary-restore path
        # lives in _provider_fallback_active/_provider_fallback_route instead.
        agent = _make_agent(
        ...
        assert agent._provider_fallback_route == ("glm-5.2", "zai")
    ...
    def test_records_sequential_switches_in_order(self):
        ...
        # Provenance always tracks the LATEST hop; no pending notice is staged
        # (each hop already produced its single announce line).
        assert getattr(agent, "_pending_fallback_notice", None) is None
        assert agent._provider_fallback_active is True
        assert agent._provider_fallback_route == ("deepseek-v4-flash", "deepseek")
