...
class TestRunConversation:
    ...
    def test_length_empty_content_without_think_tags_retries_normally(self, agent):
        ...
        # (parity 2026-08-30: upstream widened the continuation budget to 4
        # attempts — see conversation_loop's "after 4 continuation attempts"),
        # not immediately fire thinking-exhaustion.
    ...
    def test_truncated_tool_json_after_tool_batch_closes_tool_tail(self, agent):
        ...
        Migrated from the former monolithic tests/run_agent/test_run_agent.py
        (upstream a7a696ba5) during the 2026-07-23 parity merge: the fork split
        the monolith into per-issue files and this tool-tail-closing case had no
        equivalent in the split suite.
        """
        self._setup_agent(agent)
        agent.valid_tool_names.add("write_file")
        good_tc = _mock_tool_call(
    ...
    def test_length_truncated_tool_exhaustion_after_tool_batch_closes_tool_tail(self, agent):
        ...
        Migrated from the former monolithic tests/run_agent/test_run_agent.py
        (upstream a7a696ba5) during the 2026-07-23 parity merge.
        """
        self._setup_agent(agent)
        agent.valid_tool_names.add("write_file")
        good_tc = _mock_tool_call(
