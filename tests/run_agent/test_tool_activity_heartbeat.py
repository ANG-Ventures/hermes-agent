...
def test_heartbeat_touches_periodically_and_stops():
    ...
    thread.start()
    # Event-based sync (flake policy): wait for the cadence to prove itself
    # instead of assuming a quiet runner honors a 0.12s sleep budget.
    assert got_two.wait(timeout=5.0), (
        f"expected periodic touches, got {len(touches)}"
    )
    stop.set()
...
def test_slow_tool_call_refreshes_activity_during_execution(monkeypatch):
    ...
    touches: list = []
    got_heartbeats = threading.Event()
    ...
    def _execute_until_heartbeats(next_args):
        # Event-based sync (flake policy): hold the tool call open until the
        # start stamp + >=2 mid-call heartbeats have landed, instead of
        # betting a fixed 0.25s sleep outruns the 0.05s cadence on a loaded
        # runner.
        assert got_heartbeats.wait(timeout=5.0), (
            f"expected mid-call heartbeats, got {len(touches)}"
        )
        return json.dumps({"ok": True})
    ...
    result = te._run_agent_tool_execution_middleware(
        ...
        tool_call_id="tc1",
        execute=_execute_until_heartbeats,
        display_index=1,
    )
...
def test_concurrent_tool_call_heartbeat(monkeypatch):
    ...
    def _invoke(name, *a, **kw):
        # Event-based sync (flake policy): hold the tool open until the
        # heartbeats prove the cadence instead of racing a fixed sleep.
        assert got_heartbeats.wait(timeout=5.0), (
            f"expected mid-call heartbeats, got {len(touches)}"
        )
