...
def test_guest_barriers_leave_synchronous_alone_when_unset(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.execute("PRAGMA synchronous=1")
        apply_durability_barriers(conn)
        # No configured database.synchronous, so the guest entry point must
        # not apply one. On Darwin the separate macOS write-barrier layer
        # (_enforce_macos_synchronous_full) still deliberately forces FULL —
        # that is the btree-corruption guard, not config application.
