...
``hermes_cli.provider_seam`` binds the provider registry containers to
copy-on-write facades at their definition sites and swaps multi-container
publications in one generation reference. These tests pin the contract the
hot-registration plugin and the fleet reader gate rely on (registry-pins v0.2
spec §5.3 / §5.7 layer 1, t1–t7), plus the Phase-1 extras: idempotency,
sparse/high N, collision ownership, both boot orders, refresh-depth cleanup,
the lazy auto-extend through the facade, the per-generation providers-list
memo, event-loop latency under a parked publisher, and the cold reader.
...
# The nine containers the seam must own (the spec's tenth, the provider-list
# cache, is the per-generation ``providers_list`` memo).
EXPECTED = {
    "_REGISTRY": "providers",
    "_ALIASES": "providers",
...
def _seam_isolation():
    models.CANONICAL_PROVIDERS[0]  # latch the lazy auto-extend before saving
...
def _delta(name: str) -> dict:
    ...
    return {
        ...
        "CANONICAL_PROVIDERS": [models.ProviderEntry(name, name, f"{name} (direct API)")],
...
def test_t2_facade_write_leaves_bound_snapshot_unchanged(name):
    ...
    if isinstance(facade, dict):
        ...
        facade.append(models.ProviderEntry(marker, marker, marker))
    else:
        facade.add(marker)
...
def test_t4_reentrant_owner_thread_read_sees_committed_generation():
    ...
    def park(stage: str) -> None:
        if stage == "swap":
            ...
            observed["facade"] = name in models._PROVIDER_LABELS
...
def test_removal_methods_raise_and_copies_are_plain():
    registry = hermes_cli.auth.PROVIDER_REGISTRY
    key = next(iter(registry))
    for op in (lambda: registry.pop(key), lambda: registry.clear(), lambda: registry.__delitem__(key)):
        with pytest.raises(TypeError):
            op()
    with pytest.raises(TypeError):
        models.CANONICAL_PROVIDERS.remove(models.CANONICAL_PROVIDERS[0])
...
def test_providers_list_memo_is_per_generation_and_lock_free(monkeypatch):
    profile = ProviderProfile(name="seam-memo")
    provider_seam.publish({"_REGISTRY": {"seam-memo": profile}})
    fresh = provider_seam.current()
    assert fresh.providers_list is None
    ...
    def lister():
        start.wait()
        results.append(providers.list_providers())
    ...
    assert sum(counting.acquired.values()) == 0
    assert any(p is profile for p in results[0])
    assert fresh.providers_list is not None

    results[0].clear()  # a caller mutating its copy cannot corrupt the memo
    assert any(p is profile for p in providers.list_providers())

    later = ProviderProfile(name="seam-memo-2")
    providers.register_provider(later)
    assert any(p is later for p in providers.list_providers())
...
def test_event_loop_reads_complete_while_publisher_holds_the_lock(monkeypatch):
    models.CANONICAL_PROVIDERS[0]
    assert models._canonical_extended is True, "precondition: auto-extend latched"
    counting = _CountingLock()
    monkeypatch.setattr(provider_seam, "_lock", counting)
    parked, release = threading.Event(), threading.Event()
    holder: dict = {}
...
def test_cold_reader_takes_the_lock_exactly_once_and_sees_one_generation():
    out = _run_child("""
        import threading, collections
        import providers
        from providers import ProviderProfile
        providers._discovered = True
        providers.register_provider(ProviderProfile(name="seamcold", display_name="Seam Cold"))
        from hermes_cli import provider_seam
        import hermes_cli.models as models
        ...
        class Counting:
            def __init__(self):
                self.l = threading.RLock(); self.n = collections.Counter()
                self.skip = None; self.attempt = threading.Event()
            def __enter__(self):
                i = threading.get_ident(); self.n[i] += 1
                if i != self.skip: self.attempt.set()
                self.l.acquire(); return self
            def __exit__(self, *a): self.l.release()

        lock = Counting(); provider_seam._lock = lock
        parked, release = threading.Event(), threading.Event(); pub = {}
        def park(stage):
            if stage == "swap" and threading.get_ident() == pub.get("ident"):
                parked.set(); release.wait(30)
        provider_seam._park = park
        def publisher():
            pub["ident"] = threading.get_ident()
            provider_seam.publish({"_PROVIDER_MODELS": {"seamcold-unrelated": ["m"]}})
        p = threading.Thread(target=publisher); p.start(); assert parked.wait(30)
        lock.skip = pub["ident"]
        out = {}
        def reader():
            out["slugs"] = [e.slug for e in models.CANONICAL_PROVIDERS]
        r = threading.Thread(target=reader)
        r.start()
        assert lock.attempt.wait(30), "cold reader never reached the seam lock"
        assert "slugs" not in out, "cold read finished while the lock was held"
        release.set(); p.join(30); r.join(30)
        g = provider_seam.snapshot()
        surfaces = ("seamcold" in [e.slug for e in g.CANONICAL_PROVIDERS],
                    "seamcold" in g._PROVIDER_LABELS, "seamcold" in g._KNOWN_PROVIDER_NAMES)
        print("ACQ", lock.n[r.ident], "IN_READ", "seamcold" in out["slugs"], "SURFACES", surfaces)
    """)
    assert "ACQ 1 IN_READ True SURFACES (True, True, True)" in out, out


# ---------------------------------------------------------------------------
# Boot orders and the lazy auto-extend through the facade
# ---------------------------------------------------------------------------
...
def test_lazy_auto_extend_fires_through_facade_and_snapshot():
    out = _run_child("""
        import providers
        from providers import ProviderProfile
        providers._discovered = True
        providers.register_provider(ProviderProfile(name="seamlazy", display_name="Seam Lazy"))
        from hermes_cli import provider_seam
        import hermes_cli.models as models
        g = provider_seam.snapshot()  # a snapshot latches the same trigger a facade read does
        print("SNAP", "seamlazy" in [e.slug for e in g.CANONICAL_PROVIDERS],
              g._PROVIDER_LABELS.get("seamlazy"), "seamlazy" in g._KNOWN_PROVIDER_NAMES,
              "FACADE", "seamlazy" in [e.slug for e in models.CANONICAL_PROVIDERS],
