...
def test_profile_scoped_agent_build_starts_mcp_discovery_in_profile_home(
    ...
    # build thread and assert those directly. (A bare `ready.wait(timeout=5)`
    # failed twice on loaded slice runners with "agent_ready never set after
    # build" while the build was merely still running.)
...
def test_profile_scoped_agent_build_installs_secret_scope(monkeypatch, tmp_path):
    ...
    try:
        ...
        build_thread = session.get("_agent_build_thread")
        assert build_thread is not None, "_start_agent_build published no thread"
        build_thread.join(timeout=120)  # hang guard only, not a timing bound
        assert not build_thread.is_alive(), "agent build thread never finished"
        assert built.is_set(), "agent build thread never called _make_agent"
        assert ready.is_set(), "agent_ready never set after the build thread exited"
        assert not session.get("agent_error"), (
...
def test_history_to_messages_preserves_tool_calls_for_resume_display():
    ...
    assert server._history_to_messages(history) == [
        ...
        # livesync 2026-07-10 (SPEC P1-1/P1-4b): tool rows carry
        # tool_call_id so the desktop can correlate polled tool results
        # with their tool_calls. Upstream additionally ships the raw
        # `args` so a renderer can rebuild the full call instead of the
        # 80-char `context` preview. The merged _history_to_messages
        # emits BOTH, so this exact-equality assertion must too --
        # verified by running it (parity merge 2026-08-29).
        {
            "args": {"pattern": "resume"},
            "context": "resume",
            "name": "search_files",
...
def test_session_compress_reports_aborted_summary_without_success(monkeypatch):
    ...
    try:
        ...
        assert result["summary"]["headline"] == (
            "Compression aborted: 6 messages preserved (manual — you ran /compress)"
        )
...
def test_session_create_close_race_does_not_orphan_worker(monkeypatch):
    ...
    # Scoping the assertions below to own_key keeps them robust against a daemon
    # build thread leaked from a sibling session.create test appending ITS OWN key
    # to these process-wide lists. Read it off the response rather than
    # _sessions[sid] so the capture cannot race the close that pops the entry.
    ...
    # The build thread must never pre-warm a slash worker (each worker forks the
    # full stdio MCP fleet; spawn is on-demand in slash.exec) — so there is no
    # worker to close on the orphan path either. Asserting nothing was CREATED
    # is the stronger, count-free form of the old "exactly one was closed".
    ...
    # a bare `>= 1`. (Fork and upstream invented this same fix independently.)
