...
@pytest.mark.skipif(sys.platform == "win32", reason="POSIX FD_SETSIZE behaviour")
def test_high_fd_background_process_output_is_captured(burn_fds, tmp_path):
    """The background-process reader (process_registry) has the same select() ceiling."""
    import time

    from tools.process_registry import ProcessRegistry

    registry = ProcessRegistry()
    session = registry.spawn_local("echo BG_HIGH_FD_OK", cwd=str(tmp_path))
    deadline = time.monotonic() + 15
    while time.monotonic() < deadline:
        if "BG_HIGH_FD_OK" in (registry.read_log(session.id).get("output") or ""):
            break
        time.sleep(0.1)
    assert "BG_HIGH_FD_OK" in (registry.read_log(session.id).get("output") or "")
