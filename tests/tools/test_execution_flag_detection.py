...
def _clear_reboot_optout(monkeypatch):
    ...
    Same ambient-env guard as ``tests/tools/test_hardline_blocklist.py``: the
    reboot/shutdown family downgrades out of the hardline floor when
    HERMES_ALLOW_REBOOT is truthy (fork env-gated opt-out for fleet/ops agents
    that legitimately reboot hosts — `74ed8a85c`). The chained-command tests
    here (`test_grep_pattern_operand_never_masks_a_later_command`) assert the
    *unconditional* hardline block on `&& reboot` / `| shutdown -h now`, so they
    must not inherit a truthy value from the ambient process env (the live
    gateway/fleet sets HERMES_ALLOW_REBOOT=1, which leaks into pytest and
    silently downgrades those cases → spurious failures).
    """
...
def _run_exec_option_probe(tool, args, stdin, needs_tty, tmp_path, helper_name, tag):
    """Run ``tool`` with ``helper_name`` substituted for the payload name.

    Returns ``(executed, combined_output)``.
    """
    marker = tmp_path / f"marker-{tag}"
    helper = tmp_path / helper_name
    helper.write_text("#!/bin/sh\nprintf executed > \"$MARKER\"\ncat\n")
    helper.chmod(0o755)
    input_file = tmp_path / f"probe-input-{tag}.txt"
    ...
    resolved = [
        arg.format(input=str(input_file)).replace("-payload-marker", helper_name)
        for arg in args
    ]
    env = {
        **os.environ,
    ...
    return marker.exists(), f"{completed.stdout}\n{completed.stderr}"


def _exec_option_probe_verdict(tool, args, stdin, needs_tty, tmp_path):
    """Classify whether the leading-dash assertion below is MEANINGFUL here.

    The test proves a binary does not reparse a ``-leading-dash`` operand as an
    option, by checking the payload still executed. That proof only has teeth on
    a platform+build whose ``tool`` (a) supports the exec option at all and
    (b) has the modern argv grammar the approval detector models. Two ways it
    can be vacuous rather than failing, both third-party-binary facts with zero
    of our code on the path:

    * **Option unsupported** — BSD ``man`` has no ``--pager``/``-P <program>``
      exec hand-off; macOS ``script`` lacks ``-qec``; an old ripgrep predates
      ``--hostname-bin``. A normally-named helper does not execute either.
    * **Old build reparses** — e.g. ripgrep 0.10.0 (2018) accepts ``--pre`` with
      a normal helper but its arg parser *rejects* ``--pre -payload-marker``
      outright (``Found argument '-y'``). Nothing is executed, so the exec-flag
      surface the detector guards is not reachable on this build: the detector
      is merely CONSERVATIVE there (a false positive, never a missed danger).

    Returns ``None`` when the case is meaningful (run the real assertion), or a
    skip reason string otherwise.
    """
    plain_ran, _ = _run_exec_option_probe(
        tool, args, stdin, needs_tty, tmp_path, "plainmarker", "plain"
    )
    if not plain_ran:
        return (
            f"this platform's {tool} does not honor the exec option in {args!r} "
            f"even with a normally-named helper (BSD man has no --pager/-P exec "
            f"hand-off; macOS script lacks -qec; older ripgrep predates "
            f"--hostname-bin) — the leading-dash assertion would be vacuous here"
        )
    dash_ran, output = _run_exec_option_probe(
        tool, args, stdin, needs_tty, tmp_path, "-dashprobe-marker", "dash"
    )
    if not dash_ran and re.search(r"(?i)\b(usage|unexpected|isn't valid|illegal option)\b", output):
        return (
            f"this build of {tool} supports the option but its arg parser "
            f"REJECTS a leading-dash operand ({output.strip().splitlines()[0][:80]!r}) "
            f"— the exec surface is unreachable on this build, so the detector is "
            f"only conservative here, never unsafe"
        )
    return None
...
def test_real_binaries_execute_leading_dash_program_payload(
    ...
    _skip_reason = _exec_option_probe_verdict(tool, args, stdin, needs_tty, tmp_path)
    if _skip_reason:
