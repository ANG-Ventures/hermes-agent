...
class TestSpecialFileKind:
    ...
    def test_socket(self, tmp_path, monkeypatch):
        # AF_UNIX paths are capped (~104 bytes on macOS) and pytest tmp dirs
        # can exceed that — bind a RELATIVE path from inside tmp_path so the
        # sun_path stays short while the file still lands in the sandbox.
