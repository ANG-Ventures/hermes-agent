...
A negative-lookbehind symbol query
...
@pytest.mark.parametrize(
    "pattern",
    [
        r"(?<!def )detect_hardline_command",   # negative lookbehind
...
def test_search_files_tool_path_uses_pcre2_for_lookbehind(tmp_path):
    """Through the real ShellFileOperations.search path: the lookbehind query returns
    the two call sites instead of an rg parse error."""
    import shutil
    import subprocess
    ...
    if not rg:
        pytest.skip("rg not installed")
    if subprocess.run([rg, "--pcre2", "--version"], capture_output=True, timeout=30).returncode != 0:
        pytest.skip("rg built without PCRE2")
    ...
    result = ops.search(r"(?<!def )detect_hardline_command", str(tmp_path), target="content")
    assert result.error is None, result.error
    assert result.total_count == 2
    assert all(not m.content.lstrip().startswith("def ") for m in result.matches)
