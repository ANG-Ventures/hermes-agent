...
def _fast_capability(supported=True, reason=None):
    """Stand-in for hermes_cli.models.resolve_fast_mode_capability's return.

    The fork validates fast support against the FULL route (model + provider +
    api_mode) and returns a capability object with .supported/.reason/
    .request_overrides — strictly superior to upstream's model-only
    resolve_fast_mode_overrides(model_id). Tests converge onto this contract.
    """
