...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        # Defensive only. NOT a reliable backstop for in-process gateway cron:
        # ``run_job`` no longer writes ``os.environ`` (the marker is a ContextVar),
        # so if the import itself failed inside a cron job this env read returns
        # False and the caller would AUTO-APPROVE. That import failure is
        # effectively impossible (gateway.session_context imports only
        # contextvars/typing and is already live), so log loudly rather than
        # fail silently.
        logger.error(
            "get_session_env import failed in the cron approval gate; falling back "
            "to a raw env read (NOT a reliable cron backstop post-ContextVar migration)",
            exc_info=True,
        )
    ...
    for command_variant in _command_detection_variants(command):
        ...
        for pattern_re, description, quote_masked in HARDLINE_PATTERNS_COMPILED:
            ...
            if pattern_re.search(haystack):
                # Opt-in escape hatch (fork): when HERMES_ALLOW_REBOOT is set,
                # the reboot/shutdown family downgrades out of the hardline
                # floor and is handled by the DANGEROUS_PATTERNS layer instead.
...
DANGEROUS_PATTERNS = [
    ...
    # \A anchor: both lookaheads already scan the whole string, so a match
    # at any offset implies one at offset 0. Unanchored, re.search retried
    # them at every offset -> O(n^2) (4k-segment benign command took ~160s).
...
def _run_approval_gate(
    *,
    ...
    if not is_cli and not is_gateway:
        ...
        # _is_cron_approval_context() (ContextVar-first, os.environ fallback) — NOT a raw
