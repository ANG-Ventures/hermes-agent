...
def _finalize(
    ...
    *,
    ...
    claimed = _begin_finalization(delegation_id, attempt_id=attempt_id)
    if claimed is None:
        return
    ...
    if event_record.get("recover_on_shutdown") and status != "completed":
        ...
        return
    expected_attempt = attempt_id or event_record.get("attempt_id")
    if expected_attempt:
        profile_home = (
...
def list_async_delegations() -> List[Dict[str, Any]]:
    ...
    with _records_lock:
        ...
        for r in _records.values():
            ...
            if status in ("running", "stalling"):
                ...
                if callable(fn):
                    samplers[str(r.get("delegation_id"))] = fn
            if status in ("stalling", "stalled"):
                for src, dst in (
                    ...
                    if r.get(src) is not None:
                        item[dst] = r.get(src)
            live[str(r.get("delegation_id"))] = item
    try:
    ...
    for item in items:
        fn = samplers.get(str(item.get("delegation_id")))
        if fn is None:
            continue
        try:
            token, in_tool = fn()
