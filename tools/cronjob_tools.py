...
_CRON_THREAT_PATTERNS = [
    ...
    # `\bcat[ \t]+` — word boundary + same-line gap. Without them, any word ending
    # in "cat" (pipecat, concat, tomcat) followed anywhere later by a .env-shaped
    # filename produces a fake secrets-read hit. Mirrors the identical fix in
    # tools/threat_patterns.py (papercut pc-49dc8417).
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            # parity 2026-08-30: reasoning_effort DELIBERATELY absent from the
            # model-facing schema (fork policy pin: models never choose model
            # config; CLI is the only mutation surface — same pattern as
            # model/provider/base_url). Upstream added it here; fork policy wins.
        },
        "required": ["action"]
    }
}
...
def _cronjob_tool_handler(args: Dict[str, Any], **kw: Any) -> str:
    ...
    Also resolves the one model-facing ``monitor`` field into the stored
