...
def _is_line_oriented_newline_error(error: Optional[str]) -> bool:
    ...
    newer rg emits it with a `-U/--multiline` hint). Match on the
    version-stable core so the guard works regardless of which rg is on PATH.
    """
    if not error:
        return False
    lowered = error.lower()
...
class ShellFileOperations(FileOperations):
    ...
    def write_file(self, path: str, content: str,
        ...
        # already performs.
        #
        # Skipped when the sha256 check above already proved a byte-exact
        # persist: this text-mode re-read decodes the shell's stdout, which
        # cannot round-trip surrogateescape (binary-ish) content and would
        # report a false mismatch on a write the hash verified. It remains
        # the fallback for backends without sha256sum (content_verified is
        # None), incl. Windows CRLF translation handled by the normalization.
