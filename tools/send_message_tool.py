...
def _handle_react(args, remove=False):
    ...
    if not chat_id:
        # Inside a live messaging turn a bare target means the turn's own channel; only
        # cron/CLI/TUI/desktop/ACP contexts fall back to the platform home channel.
...
def _handle_send(args):
    ...
    used_home_channel = used_origin_channel = False
    if not chat_id:
        # Inside a live messaging turn a bare target means the turn's own channel (thread kept);
        # only cron/CLI/TUI/desktop/ACP contexts fall back to the platform home channel.
        state, origin_chat, origin_thread = _resolve_send_target(platform_name)
        if state == _SEND_TARGET_ORIGIN:
            chat_id, used_origin_channel = origin_chat, True
            thread_id = thread_id or origin_thread
        elif state == _SEND_TARGET_IN_TURN_UNRESOLVABLE:
            return _origin_unresolvable_error(platform_name)
        else:
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success"):
            ...
            elif used_origin_channel:
                result["note"] = f"Sent to the current {platform_name} channel (chat_id: {chat_id})"
...
# ---------------------------------------------------------------------------
# A send_message / react call with NO explicit chat target used to fall back to the
# platform's global home channel. That is right for cron/CLI/standalone sends, but inside a
# live MESSAGING turn the destination is the channel the turn is happening in: a mid-turn
# update from a non-home channel otherwise leaks into the home channel (sometimes on a
# different platform).
#
