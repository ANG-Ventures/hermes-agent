...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "shared group; pass local=true for a single-agent skill in "
        f"{display_hermes_home()}/skills/; create must precede that "
        "skill's other ops), patch (targeted old_string/new_string fix — "
        "preferred; content alone REPLACES the whole file, read it via "
        "skill_view() first), write_file/remove_file (supporting files), "
        "delete (sole op only). Existing skills are modified wherever "
        "they live. Keep the description's first 57 chars a "
        "self-contained trigger: 'Use when <trigger>. <one-line "
        "behavior>.' — skill_view() shows format conventions."
    ),
    "parameters": {
        "type": "object",
        "properties": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "category": {
                            "type": "string",
                            "description": (
                                "The skill's group/domain for create (e.g. "
                                "'devops'). When authoring into the shared "
                                "tree (the default when one exists) this "
                                "MUST be a real shared group "
                                "(skills-shared/<group>/); with local=true "
                                "it is an optional local subdirectory."
                            )
                        },
                        "local": {
                            "type": "boolean",
                            "description": (
                                "create only. Default false: when a shared "
                                "skills tree exists, new skills are authored "
                                "into skills-shared/<category>/ (git-backed, "
                                "shared across agents). Set true ONLY for a "
                                "genuine single-agent skill in the local "
                                "(gitignored, not-backed-up) tree."
                            )
                        },
            ...
            # replace_all/category/file_path/file_content/local) — old
            # transcripts and staged-write replay depend on it — plus
            # `absorbed_into` on delete ops (curator-only vocabulary; the
            # curator's prompt documents it and the delete guard's error
            # re-teaches it). None are advertised.
        },
        "required": ["operations"],
    },
}
