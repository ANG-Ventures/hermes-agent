...
Why: on 2026-09-24 a gateway went deaf for 20-30 minutes in several chat
sessions. Finding out WHY took a py-spy of the gateway process: eight turn
threads were parked in foreground terminal waits (a 59-minute polling loop, a
38-minute watch script, a 71-minute refresh). Nothing in the logs said so.

This module makes that state grep-able. Every call dispatched through
``tools.registry.ToolRegistry.dispatch`` is registered while it runs; a single
daemon thread emits one WARNING line per call at every multiple of
``TOOL_WAIT_LONG_INTERVAL_S`` (5 minutes) it is still outstanding:

    PHASE=tool_wait_long tool=terminal elapsed=600s session=agent:main:... thread=... cmd='for i in ...'

and one ``PHASE=tool_wait_long_done`` line when a reported call finishes.
Calls that finish before the first interval cost a dict insert and delete.
"""
...
def _session_key() -> str:
    try:
        ...
        return _current_session_key() or "-"
    except Exception:
        return "-"
