...
def _load_local_whisper_model(model_name: str, device: str = "auto", compute_type: str = "auto"):
    ...
    if force_cpu:
        # Apple Silicon/Rosetta: stay on CPU to avoid the native
        # device-autodetection crash. But honour an EXPLICIT compute_type from
        # ``stt.local.compute_type`` — the crash is about DEVICE probing, not
        # about the quantisation, so silently overriding a user's float32 pin
        # with int8 discards configuration for no safety benefit (#8319 is the
        # regression that these values must reach WhisperModel at all).
