...
def _(rid, params: dict) -> dict:
    ...
    source = _resolve_session_source(
        _sanitize_client_source(raw_source)
    ...
    try:
        ...
        try:
            ...
            agent = _make_agent(
                ...
                # `source` is untrusted client input (sanitized); the trusted
                # `platform_override` lane is for internal callers.
...
def _(rid, params: dict) -> dict:
    ...
    # /undo N form and session.redo can share one implementation. Upstream's
    # "skip display_kind bookkeeping rows" fix lives in hermes_undo's
    # half-turn boundary logic (_party/compute_half_turn_target).
    try:
        ...
        try:
            ...
            try:
                agent = _make_agent(
                    ...
                    # A branch inherits its parent's client label unless the
                    # caller names one explicitly, so a forked chat stays
                    # attributed to the desktop/dashboard it was branched from.
                    # Routed through `source` (untrusted client input, hence
                    # sanitized) rather than the trusted `platform_override`.
                    source=_sanitize_client_source(
