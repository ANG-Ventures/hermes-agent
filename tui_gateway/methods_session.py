        ...
        try:
            ...
            try:
                ...
                agent = _make_agent(
                    ...
                    session_id=target,
                    session_db=db,
                    # Prefer the client's declared source on this resume; else
                    # keep the label the session was persisted with (never
                    # silently relabel a desktop/dashboard session to "tui" on
                    # resume). `source` is untrusted client input (sanitized);
                    # the trusted `platform_override` lane is for internal
                    # callers.
