...
_HEARTBEAT_REFRESH_S = max(
    0.0, float(os.environ.get("HERMES_GATEWAY_HEARTBEAT_REFRESH_S") or 60.0)
)
...
def _make_agent(
    ...
    return AIAgent(
        ...
        # ``platform_override`` is an internal, trusted caller argument (resolved
        # with env fallback); ``source`` is UNTRUSTED client input and MUST go
        # through the slug guard so a hostile/malformed label can't become a
        # persisted platform. ``None`` (stdio Ink, internal rebuilds) → "tui".
        platform=(
            _resolve_agent_platform(platform_override)
            if platform_override
            else _sanitize_client_source(source)
        ),
...
def _reload_session_history_from_db(session: dict) -> list[dict]:
    ...
    # repair_alternation: this reload feeds LIVE REPLAY — session["history"] is
    # the working conversation for subsequent turns, and a rewind that lands on
    # a durable user;user pair would otherwise re-fire the pre-request repair on
    # every request from here on.
    # include_row_ids: durable rewind targeting addresses messages by their
    # stamped ``_row_id`` (upstream 79b7d969d3 rejects unstamped ordinal
    # rewinds) — a reload that drops the stamps would make every message in
    # the refreshed history untargetable for the next /undo.
    active = db.get_messages_as_conversation(
        session["session_key"], repair_alternation=True, include_row_ids=True
    )
    with session["history_lock"]:
