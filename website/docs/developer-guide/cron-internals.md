...
`_job_script_kwargs(job)` then narrows it per job via `scheduler_ext.resolve_job_script_timeout()`: explicit `timeout_s` → schedule interval (`interval` minutes, or the width of the current `cron` slot; floor 60 s) → the global value. The per-job result is always clamped to the global cap. A timeout logs `PHASE=cron_script_timeout`.
