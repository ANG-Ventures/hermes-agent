...
| `HERMES_KANBAN_DB` | Pin the kanban database file path directly (highest precedence; beats `HERMES_KANBAN_BOARD` and `HERMES_KANBAN_HOME`). The dispatcher injects this into worker subprocess env so profile workers converge on the dispatcher's board. **Redirecting `HERMES_HOME` does NOT sandbox kanban** — the pin outranks it. When an inherited pin escapes a redirected `HERMES_HOME`, or any pin reaches the machine's real `~/.hermes` while `HERMES_HOME` points elsewhere, kanban now raises `KanbanPinDivergenceError` instead of silently writing to the live board. Use `HERMES_KANBAN_SANDBOX=1` to isolate |
