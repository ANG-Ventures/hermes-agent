...
| `kanban_attach` | Attach a file to a task. Prefer `path` (absolute path on the agent's host; read server-side, byte-exact). `content_base64` is a fallback for content with no file and requires `expected_sha256`; the model re-emits base64 token by token and alters long payloads. Stored under the task's attachments dir, capped at 25 MB; returns the stored SHA-256 and stores nothing on a digest mismatch. | `HERMES_KANBAN_TASK` or `kanban` toolset |
