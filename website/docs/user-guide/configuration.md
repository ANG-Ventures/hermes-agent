...
compression:
  ...
  context_timeout_seconds: 120                      # Minimum inactivity budget for in-agent compress_context (loop /compress / preflight) — see below
...
`context_timeout_seconds` (default `120`) is the minimum **inactivity budget** for in-agent `compress_context` — the conversation loop, preflight compaction, and manual `/compress` — so a hung summary model cannot stall a session indefinitely. Positive values are clamped just above the effective `auxiliary.compression.timeout`; this lets the inner transport timeout raise and enter its configured fallback chain instead of being preempted by the outer host fence. Streamed summary tokens and fallback transitions extend the wait; only a silent worker is cut off. On timeout Hermes skips compaction, keeps the existing messages, and warns the user. Set to `0` to disable. Gateway session hygiene keeps its own `hygiene_timeout_seconds` path and is not double-wrapped.

`context_total_ceiling_seconds` (default `600`) bounds the in-agent **pre-commit** wait (summary / stream phase) even while tokens are still moving. It is clamped to at least the resolved context timeout after the auxiliary deadline floor is applied. The exact guarantee: **the summary phase is bounded by this ceiling; the commit phase is logged and surfaced if it exceeds it.** Once the worker has entered the compression commit fence and SessionDB mutation is in flight, the commit is never abandoned mid-flight — that would risk transcript divergence — but the wait is no longer silent: Hermes logs the overrun (WARNING, escalating to ERROR on repeats), sends a one-time warning through the user-visible channel, and continues waiting in bounded increments until the commit finishes.

:::tip Recovering a session that keeps timing out
`/compress` bypasses the automatic compression cooldown, but it uses the same resolved `context_timeout_seconds` and `context_total_ceiling_seconds` as other in-agent compaction paths. If compression still times out after the auxiliary fallback chain runs, check `auxiliary.compression` health or raise those budgets before retrying.
:::
