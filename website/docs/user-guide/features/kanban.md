...
| `kanban_attach` | Attach a file to a task from a host `path` (preferred, byte-exact) or inline base64 with `expected_sha256`; stored under the task's attachments dir (25 MB cap). | path, or bytes + name + sha256 |
